/* OpenRemotePlay (ORP) v2 — MIT License — ESM Bundle */
...
async function hmacSha256(key, data) {
  try {
    ...
    const keyData = encoder4.encode(key);
    ...
    const signature = await crypto.subtle.sign("HMAC", cryptoKey, encoder4.encode(data));
...
var ORPClient = class extends EventEmitter {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws.onopen = async () => {
        ...
        this.pc.onconnectionstatechange = () => {
          var _a, _b2, _c2;
          if (((_a = this.pc) == null ? void 0 : _a.connectionState) === "disconnected" || ((_b2 = this.pc) == null ? void 0 : _b2.connectionState) === "failed" || ((_c2 = this.pc) == null ? void 0 : _c2.connectionState) === "closed") {
    ...
    return new Promise((resolve, reject) => {
      ...
      const onchange = () => {
        ...
        if (((_a = this.pc) == null ? void 0 : _a.connectionState) === "connected") {
          ...
          resolve();
        } else if (((_b2 = this.pc) == null ? void 0 : _b2.connectionState) === "failed") {
          this.pc.removeEventListener("connectionstatechange", onchange);
    ...
    return new Promise((resolve, reject) => {
      ...
      const bindDc = (dc2) => {
        if (dc2.readyState === "open") {
          ...
          dc2.onerror = () => reject(new Error("data-channel-failed"));
        }
      };
      if (this.dc) {
        bindDc(this.dc);
  ...
  _cleanup() {
    ...
    try {
      (_b2 = this.pc) == null ? void 0 : _b2.close();
    } catch {
    }
    try {
      (_c2 = this.ws) == null ? void 0 : _c2.close(1e3, "cleanup");
    } catch {
    }
    this.dc = null;
    this.pc = null;
...
var ORPHostSession = class {
  ...
  async _onViewerJoin(senderId, displayName, color, ws, timing) {
    ...
    const inputChannel = pc2.createDataChannel("orp-input", { ordered: false, maxRetransmits: 0 });
    const videoChannel = pc2.createDataChannel("orp-video", { ordered: false, maxRetransmits: 0 });
    ...
    pc2.addEventListener("icecandidate", async (ev) => {
  ...
  _removeViewer(senderId) {
    ...
    if (v) {
      ...
      try {
        (_b2 = v.videoChannel) == null ? void 0 : _b2.close();
      } catch {
      }
      try {
        v.pc.close();
  ...
  broadcastFrame(isKey, timestamp, data, config) {
    ...
    for (const [, v] of this.viewers) {
      const dc2 = v.videoChannel;
      if (!dc2 || dc2.readyState !== "open") continue;
      const BLOAT_LIMIT = 2 * 1024 * 1024;
      if (dc2.bufferedAmount > BLOAT_LIMIT && !isKey) continue;
      if (isKey && config) {
        const cfgBytes = new TextEncoder().encode(config);
        const cfgBuf = new Uint8Array(1 + cfgBytes.length);
        cfgBuf[0] = 0;
        cfgBuf.set(cfgBytes, 1);
        dc2.send(cfgBuf.buffer);
      }
      dc2.send(frame.buffer);
    }
  }
...
var getRelays = (config, defaults, defaultN, deriveFromAppId = false) => {
  var _a, _b2;
  return ((_a = config.relayConfig) == null ? void 0 : _a.urls) || (deriveFromAppId ? shuffle(defaults, strToNum(config.appId)) : defaults).slice(0, ((_b2 = config.relayConfig) == null ? void 0 : _b2.redundancy) ?? defaultN);
};
...
var peer_default = (initiator, { trickleIce, rtcConfig, rtcPolyfill, turnConfig, _test_only_mdnsHostFallbackToLoopback }) => {
  ...
  const localDescriptionSignal = (peerConnection) => {
    ...
    return {
      ...
      sdp: normalizeSdp(((_b2 = peerConnection.localDescription) == null ? void 0 : _b2.sdp) ?? "")
    };
  };
  const getRemoteUfrag = () => {
    var _a, _b2;
    const sdp = (_a = pc2.remoteDescription) == null ? void 0 : _a.sdp;
    if (!sdp) return null;
    return ((_b2 = sdp.match(/a=ice-ufrag:([^\s]+)/)) == null ? void 0 : _b2[1]) ?? null;
  };
  const getRemoteMediaSectionCount = () => {
    var _a, _b2;
    return (((_b2 = (_a = pc2.remoteDescription) == null ? void 0 : _a.sdp) == null ? void 0 : _b2.match(/^m=/gm)) ?? []).length;
  };
  ...
  const createOffer = async (restartIce = false) => {
    ...
    try {
      makingOffer = true;
      if (restartIce) {
        if (pc2.signalingState !== "stable" && pc2.signalingState !== "closed" && ((_a = pc2.localDescription) == null ? void 0 : _a.type) === offerType) await pc2.setLocalDescription({ type: "rollback" });
      ...
      (_b2 = handlers.error) == null ? void 0 : _b2.call(handlers, toError(err, "failed to create local offer"));
    } finally {
      makingOffer = false;
    }
  };
  ...
  pc2.ontrack = (e) => {
    ...
    if (stream) {
      ...
      (_b2 = handlers.stream) == null ? void 0 : _b2.call(handlers, stream);
    }
  };
  pc2.onremovestream = (e) => {
    var _a;
  ...
  if (initiator) queueMicrotask(() => {
    var _a;
    if (!makingOffer && pc2.signalingState === "stable" && !pc2.localDescription && pc2.connectionState !== "closed") (_a = pc2.onnegotiationneeded) == null ? void 0 : _a.call(pc2, new Event("negotiationneeded"));
  });
  return {
    created: Date.now(),
    connection: pc2,
    ...
    getOffer: async (restartIce = false) => {
      ...
      if (((_a = pc2.localDescription) == null ? void 0 : _a.type) === offerType) return shouldTrickleIce ? localDescriptionSignal(pc2) : waitForIceGathering(pc2);
      return offerPromise;
    },
    async signal(sdp) {
      var _a, _b2, _c2;
      ...
      if ((dataChannel == null ? void 0 : dataChannel.readyState) === "open" && !((_b2 = sdp.sdp) == null ? void 0 : _b2.includes("a=rtpmap"))) return;
      try {
        const rtcSdp = {
          ...sdp,
        ...
        (_c2 = handlers.error) == null ? void 0 : _c2.call(handlers, toError(err, "failed to apply remote signal"));
      }
    },
...
var createActionWireManager = ({ getPeer, getPeerIds, canReceiveFromPeer, throwIfAborted: throwIfAborted4 }) => {
  const actions = {};
  const actionsCache = {};
      ...
      requestMany: async (data, options) => {
        ...
        return await all(options.targets.map(async (target) => {
          ...
          try {
            ...
            (_b2 = options.onResult) == null ? void 0 : _b2.call(options, result);
            return result;
          }
        }));
      },
...
var createMediaManager = ({ iterate, isActive, getSharedMediaPeer }) => {
  ...
  const emitStream = (id, key, stream, metadata) => {
    ...
    (_b2 = (_a = getSharedMediaPeer(id)) == null ? void 0 : _a.__trysteroMedia) == null ? void 0 : _b2.rememberRemoteStream(key, stream, typeof stream.id === "string" ? stream.id : void 0);
    (_c2 = listeners.onPeerStream) == null ? void 0 : _c2.call(listeners, stream, id, metadata);
  };
  const emitTrack = (id, key, track, stream, metadata) => {
    var _a, _b2, _c2;
    if (!isActive(id)) return;
    (_b2 = (_a = getSharedMediaPeer(id)) == null ? void 0 : _a.__trysteroMedia) == null ? void 0 : _b2.rememberRemoteTrack(key, track, stream, typeof track.id === "string" ? track.id : void 0, typeof stream.id === "string" ? stream.id : void 0);
    (_c2 = listeners.onPeerTrack) == null ? void 0 : _c2.call(listeners, track, stream, id, metadata);
  };
  ...
  return {
    ...
    receiveStreamMeta: (meta, id) => {
      ...
      if (!parsed) return;
      const cached = (_b2 = (_a = getSharedMediaPeer(id)) == null ? void 0 : _a.__trysteroMedia) == null ? void 0 : _b2.getRemoteStream(parsed.key, parsed.streamId);
      if (cached) {
    ...
    receiveTrackMeta: (meta, id) => {
      ...
      const parsed = toPendingMediaMeta(meta);
      if (!parsed) return;
      const cached = (_b2 = (_a = getSharedMediaPeer(id)) == null ? void 0 : _a.__trysteroMedia) == null ? void 0 : _b2.getRemoteTrack(parsed.key, parsed.trackId);
      if (cached) {
...
var SharedPeerManager = class {
  ...
  clear(appId, peerId, { destroyPeer }) {
    ...
    bindings.forEach((binding) => {
      var _a, _b2;
      (_b2 = (_a = binding.handlers).close) == null ? void 0 : _b2.call(_a);
  ...
  dispatchData(shared, data) {
    ...
    if (decoded.type === "presence") {
      ...
      (_b2 = (_a = this.roomPresenceHandlers)[shared.appId]) == null ? void 0 : _b2.call(_a, shared.peerId, decoded.roomToken, decoded.isPresent);
      return;
    }
  ...
  dispatchSignal(shared, signal) {
    var _a, _b2, _c2;
    (_c2 = (_a = this.getSignalBinding(shared)) == null ? void 0 : (_b2 = _a.handlers).signal) == null ? void 0 : _c2.call(_b2, signal);
  }
  dispatchTrack(shared, track, stream) {
    values(shared.bindings).forEach((binding) => {
      var _a, _b2, _c2, _d;
      if (binding.handlers.track || binding.handlers.stream) {
        (_b2 = (_a = binding.handlers).track) == null ? void 0 : _b2.call(_a, track, stream);
        (_d = (_c2 = binding.handlers).stream) == null ? void 0 : _d.call(_c2, stream);
        return;
      }
...
var handleAnnouncement = async (ctx, relayId, peerId, shared, signalPeer) => {
  ...
  state.offerSignalBacklog.forEach((signal) => {
    var _a, _b2;
    return (_b2 = (_a = state.offerSignalRelays)[relayId]) == null ? void 0 : _b2.call(_a, signal);
  });
};
...
var strategy_default = ({ init, subscribe, announce, deactivate }) => {
  ...
  const ensureRoomPresenceHandler = (appId) => {
    ...
    roomPresenceHandlerCleanups[appId] = sharedPeers.setRoomPresenceHandler(appId, (peerId, roomToken, isPresent) => {
      ...
      (_c2 = (_b2 = roomRegistrations[appId]) == null ? void 0 : _b2[roomId]) == null ? void 0 : _c2.attachSharedPeerToRoom(peerId, shared);
    });
  };
  ...
  return (config, roomId, callbacks) => {
    ...
    if (!didInit) {
      ...
      didInit = true;
      cleanupWatchOnline = ((_b2 = config.relayConfig) == null ? void 0 : _b2.manualReconnection) ? noOp : watchOnline();
    }
    ...
    const joinedRoom = room_default((f) => onPeerConnect = f, (id) => {
      ...
      if (registration == null ? void 0 : registration.roomToken) {
        ...
        (_b3 = roomIdsByToken[appId]) == null ? true : delete _b3[registration.roomToken];
  ...
  init: (config) => getRelays(config, defaultRelayUrls, defaultRedundancy).map((rawUrl) => {
    const client = relayManager.register(rawUrl, () => makeSocket(rawUrl, (rawData) => {
      ...
      if ((data.offer || data.answer) && topic && data.offer_id) {
        ...
        (_b2 = (_a = msgHandlers.forKey(rawUrl))[topic]) == null ? void 0 : _b2.call(_a, data);
      }
    }));
    return client.ready;
  }),
...
var big = BigInt;
var arange = (n, min4, max, msg = "bad number: out of range") => {
  if (typeof n !== "bigint")
...
var _hash = (name2) => {
  ...
  if (typeof fn !== "function")
    throw new Error("hashes." + name2 + " not set");
  return fn;
};
var callHash = (name2, a, b) => abytes(_hash(name2)(a, b), L, "digest");
var callHashAsync = async (name2, a, b) => abytes(await _hash(name2)(a, b), L, "digest");
var apoint = (p) => {
...
var koblitz = (x2) => M(M(x2 * x2) * x2 + 7n);
...
var lift_x = (x2) => {
  const c = koblitz(FpIsValidNot0(x2));
  let r = 1n;
  ...
  return new Point(x2, isEven(r) ? r : M(-r), 1n);
};
var _Point = class _Point {
  ...
  static fromAffine(ap) {
    const { x: x2, y } = ap;
    return x2 === 0n && y === 0n ? I : new _Point(x2, y, 1n);
  }
  ...
  static fromBytes(bytes) {
    ...
    const head = bytes[0];
    const x2 = sliceBytesNumBE(bytes, 1, 33);
    try {
      ...
      if (length === 65 && head === 4)
        return new _Point(x2, sliceBytesNumBE(bytes, 33, 65), 1n).assertValidity();
    } catch (error) {
...
var extpubSchnorr = (priv) => {
  ...
  const { x: x2, y } = p.assertValidity().toAffine();
  const d = isEven(y) ? d_ : modN(-d_);
  const px2 = numTo32b(x2);
  return { d, px: px2 };
};
...
var prepSigSchnorr = (message, secretKey2, auxRand) => {
  ...
  const { px: px2, d } = extpubSchnorr(secretKey2);
  return { m, px: px2, d, a: abytes(auxRand, L) };
};
var extractK = (rand) => {
  const k_ = bytesModN(rand);
  if (k_ === 0n)
    throw new Error("sign failed: k is zero");
  const { px: px2, d } = extpubSchnorr(numTo32b(k_));
  return { rx: px2, k: d };
};
var createSigSchnorr = (k, px2, e, d) => {
  return concatBytes(px2, numTo32b(modN(k + e * d)));
};
var E_INVSIG = "invalid signature produced";
var signSchnorr = (message, secretKey2, auxRand = randomBytes(L)) => {
  const { m, px: px2, d, a } = prepSigSchnorr(message, secretKey2, auxRand);
  const t = numTo32b(d ^ bytesToNumBE(taggedHash("aux", a)));
  const { rx: rx2, k } = extractK(taggedHash("nonce", t, px2, m));
  const sig = createSigSchnorr(k, rx2, challenge(rx2, px2, m), d);
  if (!verifySchnorr(sig, m, px2))
...
var signSchnorrAsync = async (message, secretKey2, auxRand = randomBytes(L)) => {
  ...
  const { rx: rx2, k } = extractK(await taggedHashAsync("nonce", t, px2, m));
  const sig = createSigSchnorr(k, rx2, await challengeAsync(rx2, px2, m), d);
  if (!await verifySchnorrAsync(sig, m, px2))
    throw new Error(E_INVSIG);
  return sig;
};
...
var _verifSchnorr = (signature, message, publicKey2, challengeFn) => {
  ...
  return callSyncAsyncFn(challengeFn(chalInput), (e) => {
    try {
      const { x: x2, y } = doubleScalarMulUns(P_, s, modN(-e)).toAffine();
      if (!isEven(y) || x2 !== r)
        return false;
      return true;
    } catch (error) {
...
var shuffle2 = (xs, seed) => {
  const a = [...xs];
  const rand = () => {
    const x2 = sin2(seed++) * 1e4;
    return x2 - floor2(x2);
  };
  let i = a.length;
  while (i) {
    const j2 = floor2(rand() * i--);
    const tmp = a[i];
    a[i] = a[j2];
    a[j2] = tmp;
  }
...
var getRelays2 = (config, defaults, defaultN, deriveFromAppId = false) => {
  var _a, _b2;
  return ((_a = config.relayConfig) == null ? void 0 : _a.urls) || (deriveFromAppId ? shuffle2(defaults, strToNum2(config.appId)) : defaults).slice(0, ((_b2 = config.relayConfig) == null ? void 0 : _b2.redundancy) ?? defaultN);
};
...
var peer_default2 = (initiator, { trickleIce, rtcConfig, rtcPolyfill, turnConfig, _test_only_mdnsHostFallbackToLoopback }) => {
  ...
  const createOffer = async (restartIce = false) => {
    ...
    try {
      makingOffer = true;
      if (restartIce) {
        if (pc2.signalingState !== "stable" && pc2.signalingState !== "closed" && ((_a = pc2.localDescription) == null ? void 0 : _a.type) === offerType2) await pc2.setLocalDescription({ type: "rollback" });
        if (typeof pc2.restartIce === "function") pc2.restartIce();
      }
      await pc2.setLocalDescription(restartIce ? await pc2.createOffer({ iceRestart: true }) : void 0);
      return await emitLocalDescriptionSignal();
    } catch (err) {
      (_b2 = handlers.error) == null ? void 0 : _b2.call(handlers, toError2(err, "failed to create local offer"));
    } finally {
      makingOffer = false;
    }
  };
  ...
  pc2.onconnectionstatechange = handleConnectionStateChange;
  pc2.addEventListener(iceConnectionStateEvent2, handleConnectionStateChange);
  pc2.ontrack = (e) => {
    var _a, _b2;
    const stream = e.streams[0];
    if (stream) {
  ...
  return {
    ...
    getOffer: async (restartIce = false) => {
      ...
      if (!initiator) return;
      if (restartIce) return createOffer(true);
      if (((_a = pc2.localDescription) == null ? void 0 : _a.type) === offerType2) return shouldTrickleIce ? localDescriptionSignal(pc2) : waitForIceGathering(pc2);
      return offerPromise;
    },
    async signal(sdp) {
      var _a, _b2, _c2;
      ...
      try {
        ...
        if (sdp.type === offerType2) {
          if (makingOffer || pc2.signalingState !== "stable" && !isSettingRemoteAnswerPending) {
            if (initiator) return;
            await all2([pc2.setLocalDescription({ type: "rollback" }), pc2.setRemoteDescription(rtcSdp)]);
          } else await pc2.setRemoteDescription(rtcSdp);
          await flushPendingRemoteCandidates();
          await pc2.setLocalDescription();
          return await emitLocalDescriptionSignal();
        ...
        (_c2 = handlers.error) == null ? void 0 : _c2.call(handlers, toError2(err, "failed to apply remote signal"));
      }
    },
    sendData: (data) => dataChannel == null ? void 0 : dataChannel.send(data),
    destroy: () => {
...
var createActionWireManager2 = ({ getPeer, getPeerIds, canReceiveFromPeer, throwIfAborted: throwIfAborted4 }) => {
  const actions = {};
  const actionsCache = {};
  const pendingTransmissions = {};
  const pendingActionPayloads = {};
...
var strategy_default2 = ({ init, subscribe, announce, deactivate }) => {
  ...
  return (config, roomId, callbacks) => {
    ...
    if (!didInit) {
      ...
      initPromises = (Array.isArray(initRes) ? initRes : [initRes]).map((value) => Promise.resolve(value));
      didInit = true;
      cleanupWatchOnline = ((_b2 = config.relayConfig) == null ? void 0 : _b2.manualReconnection) ? noOp2 : watchOnline2();
    }
    if (!isPassive && !pool.isActive) pool.warmup();
    ...
    const client = relayManager2.register(url, () => makeSocket2(url, (data) => {
      ...
      if (payload && typeof payload === "object" && "content" in payload) {
        ...
        if ((batcher == null ? void 0 : batcher.subIds.includes(subId)) && payload.tags) {
          ...
          if (topicTag == null ? void 0 : topicTag[1]) (_b2 = batcher.topics.get(topicTag[1])) == null ? void 0 : _b2(topicTag[1], content);
        }
      }
...
function tw() {
  ...
  function f(g) {
    var P2, S = o(g), b = S[0], w = S[1], _ = new r(c(g, b, w)), R = 0, T = w > 0 ? b - 4 : b, y;
    for (y = 0; y < T; y += 4) P2 = e[g.charCodeAt(y)] << 18 | e[g.charCodeAt(y + 1)] << 12 | e[g.charCodeAt(y + 2)] << 6 | e[g.charCodeAt(y + 3)], _[R++] = P2 >> 16 & 255, _[R++] = P2 >> 8 & 255, _[R++] = P2 & 255;
    return w === 2 && (P2 = e[g.charCodeAt(y)] << 2 | e[g.charCodeAt(y + 1)] >> 4, _[R++] = P2 & 255), w === 1 && (P2 = e[g.charCodeAt(y)] << 10 | e[g.charCodeAt(y + 1)] << 4 | e[g.charCodeAt(y + 2)] >> 2, _[R++] = P2 >> 8 & 255, _[R++] = P2 & 255), _;
  }
  function h(g) {
    return t[g >> 18 & 63] + t[g >> 12 & 63] + t[g >> 6 & 63] + t[g & 63];
  }
  function d(g, P2, S) {
    for (var b, w = [], _ = P2; _ < S; _ += 3) b = (g[_] << 16 & 16711680) + (g[_ + 1] << 8 & 65280) + (g[_ + 2] & 255), w.push(h(b));
    return w.join("");
  }
  function m(g) {
    for (var P2, S = g.length, b = S % 3, w = [], _ = 16383, R = 0, T = S - b; R < T; R += _) w.push(d(g, R, R + _ > T ? T : R + _));
    return b === 1 ? (P2 = g[S - 1], w.push(t[P2 >> 2] + t[P2 << 4 & 63] + "==")) : b === 2 && (P2 = (g[S - 2] << 8) + g[S - 1], w.push(t[P2 >> 10] + t[P2 >> 4 & 63] + t[P2 << 2 & 63] + "=")), w.join("");
  }
  return On;
}
function rw() {
  ...
  return ki.read = function(t, e, r, n, i) {
    ...
    var o, a, c, f = s * 8 - i - 1, h = (1 << f) - 1, d = h >> 1, m = i === 23 ? Math.pow(2, -24) - Math.pow(2, -77) : 0, g = n ? 0 : s - 1, P2 = n ? 1 : -1, S = e < 0 || e === 0 && 1 / e < 0 ? 1 : 0;
    for (e = Math.abs(e), isNaN(e) || e === 1 / 0 ? (a = isNaN(e) ? 1 : 0, o = h) : (o = Math.floor(Math.log(e) / Math.LN2), e * (c = Math.pow(2, -o)) < 1 && (o--, c *= 2), o + d >= 1 ? e += m / c : e += m * Math.pow(2, 1 - d), e * c >= 2 && (o++, c /= 2), o + d >= h ? (a = 0, o = h) : o + d >= 1 ? (a = (e * c - 1) * Math.pow(2, i), o = o + d) : (a = e * Math.pow(2, d - 1) * Math.pow(2, i), o = 0)); i >= 8; t[r + g] = a & 255, g += P2, a /= 256, i -= 8) ;
    for (o = o << i | a, f += i; f > 0; t[r + g] = o & 255, g += P2, o /= 256, f -= 8) ;
    t[r + g - P2] |= S * 128;
  }, ki;
}
function Pf() {
  ...
  function a(p, u, l) {
    ...
    if (Ze(p, ArrayBuffer) || p && Ze(p.buffer, ArrayBuffer) || typeof SharedArrayBuffer < "u" && (Ze(p, SharedArrayBuffer) || p && Ze(p.buffer, SharedArrayBuffer))) return P2(p, u, l);
  ...
  function g(p) {
    if (Ze(p, Uint8Array)) {
      let u = new Uint8Array(p);
      return P2(u.buffer, u.byteOffset, u.byteLength);
    }
    return m(p);
  }
  function P2(p, u, l) {
...
function yw() {
  ...
  s.prototype.getMaxListeners = function() {
    ...
    var M2 = y === "error", F = this._events;
    if (F !== void 0) M2 = M2 && F.error === void 0;
    else if (!M2) return false;
    if (M2) {
      var U;
    ...
    else for (var H = W.length, z = P2(W, H), v = 0; v < H; ++v) e(z[v], this, O);
    return true;
  };
  function f(T, y, O, v) {
    var M2, F, U;
    ...
    else if (typeof U == "function" ? U = F[y] = v ? [O, U] : [U, O] : v ? U.unshift(O) : U.push(O), M2 = c(T), M2 > 0 && U.length > M2 && !U.warned) {
      U.warned = true;
  ...
  function d(T, y, O) {
    var v = { fired: false, wrapFn: void 0, target: T, type: y, listener: O }, M2 = h.bind(v);
    return M2.listener = O, v.wrapFn = M2, M2;
  }
  s.prototype.once = function(y, O) {
    ...
    var v, M2, F, U, Q;
    if (a(O), M2 = this._events, M2 === void 0) return this;
    if (v = M2[y], v === void 0) return this;
    if (v === O || v.listener === O) --this._eventsCount === 0 ? this._events = /* @__PURE__ */ Object.create(null) : (delete M2[y], M2.removeListener && this.emit("removeListener", y, v.listener || O));
      ...
      if (F < 0) return this;
      F === 0 ? v.shift() : S(v, F), v.length === 1 && (M2[y] = v[0]), M2.removeListener !== void 0 && this.emit("removeListener", y, Q || O);
    }
    return this;
    ...
    if (arguments.length === 0) {
      ...
      for (M2 = 0; M2 < F.length; ++M2) U = F[M2], U !== "removeListener" && this.removeAllListeners(U);
      return this.removeAllListeners("removeListener"), this._events = /* @__PURE__ */ Object.create(null), this._eventsCount = 0, this;
    }
    if (O = v[y], typeof O == "function") this.removeListener(y, O);
    else if (O !== void 0) for (M2 = O.length - 1; M2 >= 0; M2--) this.removeListener(y, O[M2]);
    return this;
  };
  function m(T, y, O) {
    var v = T._events;
    if (v === void 0) return [];
    var M2 = v[y];
    return M2 === void 0 ? [] : typeof M2 == "function" ? O ? [M2.listener || M2] : [M2] : O ? b(M2) : P2(M2, M2.length);
  }
  ...
  function w(T, y) {
    return new Promise(function(O, v) {
      ...
      function F() {
        typeof T.removeListener == "function" && T.removeListener("error", M2), O([].slice.call(arguments));
      }
      R(T, y, F, { once: true }), y !== "error" && _(T, M2, { once: true });
    });
  }
  function _(T, y, O) {
  ...
  function R(T, y, O, v) {
    ...
    else if (typeof T.addEventListener == "function") T.addEventListener(y, function M2(F) {
      v.once && T.removeEventListener(y, M2), O(F);
    });
...
var Ut = j((gR, Ps) => {
  ...
  function Ah(t, e, r) {
    ...
    zm(t) ? (t.on("complete", m), h || t.on("abort", w), t.req ? R() : t.on("request", R)) : o && !a && (t.on("end", f), t.on("close", f)), !h && typeof t.aborted == "boolean" && t.on("aborted", w), t.on("end", P2), t.on("finish", m), e.error !== false && t.on("error", S), t.on("close", w), b ? rr.nextTick(w) : a != null && a.errorEmitted || c != null && c.errorEmitted ? h || rr.nextTick(_) : (!s && (!h || yh(t)) && (d || wh(t) === false) || !o && (!h || wh(t)) && (g || yh(t) === false) || c && t.req && t.aborted) && rr.nextTick(_);
    let T = () => {
      r = Ts, t.removeListener("aborted", w), t.removeListener("complete", m), t.removeListener("abort", w), t.removeListener("request", R), t.req && t.req.removeListener("finish", m), t.removeListener("end", f), t.removeListener("close", f), t.removeListener("finish", m), t.removeListener("end", P2), t.removeListener("error", S), t.removeListener("close", w);
    };
    if (e.signal && !b) {
      let y = () => {
        let O = r;
...
var Nn = j((Xk, wd) => {
  ...
  le.prototype.pipe = function(t, e) {
    ...
    let c, f = false;
    function h() {
      de("cleanup"), t.removeListener("close", P2), t.removeListener("finish", S), c && t.removeListener("drain", c), t.removeListener("error", g), t.removeListener("unpipe", o), r.removeListener("end", a), r.removeListener("end", b), r.removeListener("data", m), f = true, c && n.awaitDrainWriters && (!t._writableState || t._writableState.needDrain) && c();
    }
    function d() {
    ...
    t.once("close", P2);
    function S() {
      de("onfinish"), t.removeListener("close", P2), b();
    }
    t.once("finish", S);
    function b() {
...
var so = j((iB, np) => {
  ...
  function rp(t, e, r) {
    ...
    let S;
    for (let R = 0; R < t.length; R++) {
      let T = t[R], y = R < t.length - 1, O = R > 0, v = y || (r == null ? void 0 : r.end) !== false, M2 = R === t.length - 1;
      if (ep(T)) {
        let F = function(U) {
        ...
        if (v) {
          ...
          d.push(U), la(T) && M2 && o.push(Q);
        }
        T.on("error", F), la(T) && M2 && o.push(() => {
...
var wa = j((pB, lp) => {
  ...
  lp.exports = function(...e) {
    ...
    function c(g) {
      let P2 = o;
      o = null, P2 ? P2(g) : g ? a.destroy(g) : !m && !d && a.destroy();
    }
    ...
    if (a = new ao({ writableObjectMode: !!(f != null && f.writableObjectMode), readableObjectMode: !!(h != null && h.readableObjectMode), writable: d, readable: m }), d) {
      if (uo(f)) a._write = function(P2, S, b) {
        f.write(P2, S) ? b() : n = b;
      }, a._final = function(P2) {
        f.end(), i = P2;
      }, f.on("drain", function() {
        if (n) {
          let P2 = n;
    ...
    if (m) {
      if (uo(h)) h.on("readable", function() {
        ...
        let P2 = (Or(h) ? h.readable : h).getReader();
        a._read = async function() {
          for (; ; ) try {
            let { value: S, done: b } = await P2.read();
            if (!a.push(S)) return;
            if (b) {
              a.push(null);
              return;
    ...
    return a._destroy = function(g, P2) {
      !g && o !== null && (g = new F0()), s = null, n = null, i = null, o === null ? P2(g) : (o = P2, uo(h) && j0(h, g));
    }, a;
  };
});
var mp = j((AB, va) => {
      ...
      async function b() {
        try {
          for await (let w of o) {
            ...
            m += 1, pp(w, P2, g), a.push(w), f && (f(), f = null), !d && (a.length >= n || m >= r) && await new hp((_) => {
              h = _;
            });
          }
          a.push(yp);
...
var Fp = j((g2, jp) => {
  ...
  function DS(t) {
    ...
    function r(h) {
      ...
      function S(...b) {
        ...
        b[0] = b[0].replace(/%([a-zA-Z%])/g, (O, v) => {
          if (O === "%%") return "%";
          T++;
          let M2 = r.formatters[v];
          if (typeof M2 == "function") {
            let F = b[T];
            O = M2.call(w, F), b.splice(T, 1), T--;
          }
          return O;
        }), r.formatArgs.call(w, b), (w.log || r.log).apply(w, b);
      }
      return S.namespace = h, S.useColors = r.useColors(), S.color = r.selectColor(h), S.extend = n, S.destroy = r.destroy, Object.defineProperty(S, "enabled", { enumerable: true, configurable: false, get: () => m !== null ? m : (g !== r.namespaces && (g = r.namespaces, P2 = r.enabled(h)), P2), set: (b) => {
        m = b;
    ...
    function s(h, d) {
      let m = 0, g = 0, P2 = -1, S = 0;
      for (; m < h.length; ) if (g < d.length && (d[g] === h[m] || d[g] === "*")) d[g] === "*" ? (P2 = g, S = m, g++) : (m++, g++);
      else if (P2 !== -1) g = P2 + 1, S++, m = S;
      else return false;
...
var At = j((Qe, po) => {
  ...
  function $S() {
    ...
    return !t && typeof N2 < "u" && "env" in N2 && (t = N2.env.DEBUG), t;
  }
  function HS() {
    try {
      return localStorage;
...
var Qp = j((tN, Ra) => {
  ...
  x();
  I2();
  typeof N2 > "u" || !N2.version || N2.version.indexOf("v0.") === 0 || N2.version.indexOf("v1.") === 0 && N2.version.indexOf("v1.8.") !== 0 ? Ra.exports = { nextTick: ZS } : Ra.exports = N2;
  function ZS(t, e, r, n) {
...
var Ba = j((fN, ny) => {
  ...
  function aE(t, e, r) {
    ...
    let w = 0;
    return w |= h != null ? ue.USERNAME_MASK : 0, w |= d != null ? ue.PASSWORD_MASK : 0, w |= o && o.retain ? ue.WILL_RETAIN_MASK : 0, w |= o && o.qos ? o.qos << ue.WILL_QOS_SHIFT : 0, w |= o ? ue.WILL_FLAG_MASK : 0, w |= a ? ue.CLEAN_SESSION_MASK : 0, e.write(se.from([w])), He(e, c), s === 5 && P2.write(), pn(e, f), o && (s === 5 && S.write(), Br(e, o.topic), pn(e, o.payload)), h != null && pn(e, h), d != null && pn(e, d), true;
  }
  function uE(t, e, r) {
  ...
  function fE(t, e, r) {
    ...
    if (typeof a == "object" && a.length) for (let m = 0; m < a.length; m += 1) {
      let g = a[m].topic, P2 = a[m].qos;
      if (typeof g != "string") return e.destroy(new Error("Invalid subscriptions - invalid topic")), false;
      if (typeof P2 != "number") return e.destroy(new Error("Invalid subscriptions - invalid qos")), false;
      if (n === 5) {
    ...
    let d = true;
    for (let m of a) {
      let g = m.topic, P2 = m.qos, S = +m.nl, b = +m.rap, w = m.rh, _;
      Br(e, g), _ = ue.SUBSCRIBE_OPTIONS_QOS[P2], n === 5 && (_ |= S ? ue.SUBSCRIBE_OPTIONS_NL : 0, _ |= b ? ue.SUBSCRIBE_OPTIONS_RAP : 0, _ |= w ? ue.SUBSCRIBE_OPTIONS_RH[w] : 0), d = e.write(se.from([_]));
    }
    return d;
  }
  function hE(t, e, r) {
  ...
  function AE(t) {
    ...
    function s(c, f) {
      ...
      for (let m = 0; m < h.length; m++) {
        let g = h[m], P2 = c[g];
        if (typeof P2 != "object" || P2 === null) d[g] = P2;
        else if (P2.constructor !== Object && (i = n.get(P2.constructor))) d[g] = i(P2, f);
        else if (ArrayBuffer.isView(P2)) d[g] = yn(P2);
        else {
          let S = e.indexOf(P2);
          S !== -1 ? d[g] = r[S] : d[g] = f(P2);
        }
      }
      return d;
    }
...
var Mr = j((ht) => {
  ...
  ht.nextTick = typeof (N2 == null ? void 0 : N2.nextTick) == "function" ? N2.nextTick : (t) => {
    setTimeout(t, 0);
  };
...
var Ey = j((_o) => {
  ...
  _o.LRUCache = void 0;
  var gn = typeof performance == "object" && performance && typeof performance.now == "function" ? performance : Date, _y = /* @__PURE__ */ new Set(), $a = typeof N2 == "object" && N2 ? N2 : {}, vy = (t, e, r, n) => {
  ...
  var NE = (t) => !_y.has(t), gL = /* @__PURE__ */ Symbol("type"), lr = (t) => t && t === Math.floor(t) && t > 0 && isFinite(t), Sy = (t) => lr(t) ? t <= Math.pow(2, 8) ? Uint8Array : t <= Math.pow(2, 16) ? Uint16Array : t <= Math.pow(2, 32) ? Uint32Array : t <= Number.MAX_SAFE_INTEGER ? bn : null : null, bn = class extends Array {
    ...
    constructor(e) {
      ...
      let { max: r = 0, ttl: n, ttlResolution: i = 1, ttlAutopurge: s, updateAgeOnGet: o, updateAgeOnHas: a, allowStale: c, dispose: f, disposeAfter: h, noDisposeOnSet: d, noUpdateTTL: m, maxSize: g = 0, maxEntrySize: P2 = 0, sizeCalculation: S, fetchMethod: b, memoMethod: w, noDeleteOnFetchRejection: _, noDeleteOnStaleGet: R, allowStaleOnFetchRejection: T, allowStaleOnFetchAbort: y, ignoreFetchAbort: O } = e;
      if (r !== 0 && !lr(r)) throw new TypeError("max option must be a nonnegative integer");
      let v = r ? Sy(r) : Array;
      if (!v) throw new Error("invalid max value: " + r);
      if (__privateSet(this, _u2, r), __privateSet(this, _f2, g), this.maxEntrySize = P2 || __privateGet(this, _f2), this.sizeCalculation = S, this.sizeCalculation) {
      ...
      if (!this.ttlAutopurge && !__privateGet(this, _u2) && !__privateGet(this, _f2)) {
        let M2 = "LRU_CACHE_UNBOUNDED";
        NE(M2) && (_y.add(M2), vy("TTL caching without ttlAutopurge, max, or maxSize can result in unbounded memory consumption.", "UnboundedCacheWarning", M2, _e2));
      }
    }
    ...
    *rvalues() {
    ...
    async fetch(e, r = {}) {
      let { allowStale: n = this.allowStale, updateAgeOnGet: i = this.updateAgeOnGet, noDeleteOnStaleGet: s = this.noDeleteOnStaleGet, ttl: o = this.ttl, noDisposeOnSet: a = this.noDisposeOnSet, size: c = 0, sizeCalculation: f = this.sizeCalculation, noUpdateTTL: h = this.noUpdateTTL, noDeleteOnFetchRejection: d = this.noDeleteOnFetchRejection, allowStaleOnFetchRejection: m = this.allowStaleOnFetchRejection, ignoreFetchAbort: g = this.ignoreFetchAbort, allowStaleOnFetchAbort: P2 = this.allowStaleOnFetchAbort, context: S, forceRefresh: b = false, status: w, signal: _ } = r;
      if (!__privateGet(this, _x2)) return w && (w.fetch = "get"), this.get(e, { allowStale: n, updateAgeOnGet: i, noDeleteOnStaleGet: s, status: w });
      let R = { allowStale: n, updateAgeOnGet: i, noDeleteOnStaleGet: s, ttl: o, noDisposeOnSet: a, size: c, sizeCalculation: f, noUpdateTTL: h, noDeleteOnFetchRejection: d, allowStaleOnFetchRejection: m, allowStaleOnFetchAbort: P2, ignoreFetchAbort: g, status: w, signal: _ }, T = __privateGet(this, _n).get(e);
      if (T === void 0) {
        w && (w.fetch = "miss");
    ...
    let g = new Promise(m).then(f, h), P2 = Object.assign(g, { __abortController: o, __staleWhileFetching: s, __returned: void 0 });
    return r === void 0 ? (this.set(e, P2, { ...c.options, status: void 0 }), r = __privateGet(this, _n).get(e)) : __privateGet(this, _e)[r] = P2, P2;
  }, t_fn = function(e) {
...
var zy = j((Qn) => {
  ...
  var j1 = zn(), F1 = D1(Gy2()), W1 = Mr(), $1 = (t, e) => {
...
var Pu = j((Fo, Xy) => {
  ...
  (function(t, e) {
    ...
    var e = function(m) {
      ...
      return function(g, P2) {
        return m.set(g, P2), P2;
      };
    }, n = Number.MAX_SAFE_INTEGER === void 0 ? 9007199254740991 : Number.MAX_SAFE_INTEGER, i = 536870912, s = i * 2, o = function(m, g) {
      return function(P2) {
        var S = g.get(P2), b = S === void 0 ? P2.size : S < s ? S + 1 : 0;
        if (!P2.has(b)) return m(P2, b);
        if (P2.size < i) {
          for (; P2.has(b); ) b = Math.floor(Math.random() * s);
          return m(P2, b);
        }
        if (P2.size > n) throw new Error("Congratulations, you created a collection of unique numbers which uses all available integers!");
        for (; P2.has(b); ) b = Math.floor(Math.random() * n);
        return m(P2, b);
      };
...
var Cu = j((AW, Ht) => {
  ...
  function hg() {
    ...
    function s(g, P2, S, b) {
      var w = P2 && P2.prototype instanceof a ? P2 : a, _ = Object.create(w.prototype);
      return Rt(_, "_invoke", (function(R, T, y) {
        var O, v, M2, F = 0, U = y || [], Q = false, W = { p: 0, n: 0, v: t, a: H, f: H.bind(t, 4), d: function(J, ee) {
          return O = J, v = 0, M2 = t, W.n = ee, o;
        } };
        function H(z, J) {
          for (v = z, M2 = J, e = 0; !Q && F && !ee && e < U.length; e++) {
            var ee, G2 = U[e], V = W.p, te = G2[2];
            z > 3 ? (ee = te === J) && (M2 = G2[(v = G2[4]) ? 5 : (v = 3, 3)], G2[4] = G2[5] = t) : G2[0] <= V && ((ee = z < 2 && V < G2[1]) ? (v = 0, W.v = J, W.n = G2[1]) : V < te && (ee = z < 3 || G2[0] > J || J > te) && (G2[4] = z, G2[5] = J, W.n = te, v = 0));
          }
          if (ee || z > 1) return o;
          throw Q = true, J;
        }
        return function(z, J, ee) {
          ...
          for (Q && J === 1 && H(J, ee), v = J, M2 = ee; (e = v < 2 ? t : M2) || !Q; ) {
            O || (v ? v < 3 ? (v > 1 && (W.n = -1), H(v, M2)) : W.n = M2 : W.v = M2);
            try {
              if (F = 2, O) {
                if (v || (z = "next"), e = O[z]) {
                  if (!(e = e.call(O, M2))) throw TypeError("iterator result is not an object");
                  if (!e.done) return e;
                  M2 = e.value, v < 2 && (v = 0);
                } else v === 1 && (e = O.return) && e.call(O), v < 2 && (M2 = TypeError("The iterator does not provide a '" + z + "' method"), v = 1);
                O = t;
              } else if ((e = (Q = W.n < 0) ? M2 : R.call(T, W)) !== o) break;
            } catch (G2) {
              O = t, v = 1, M2 = G2;
            } finally {
              F = 1;
...
var vg = j((Ho, _g) => {
  ...
  (function(t, e) {
      ...
      return function(T) {
        ...
        return function(O) {
          ...
          O.addEventListener("message", function(ee) {
            var G2 = ee.data, V = G2.id;
            if (V !== null && v.has(V)) {
              var te = v.get(V), Y = te.reject, X = te.resolve;
              v.delete(V), G2.error === void 0 ? X(G2.result) : Y(new Error(G2.error.message));
            }
          }), R(O) && O.start();
          for (var M2 = function(G2) {
            ...
            return new Promise(function(Y, X) {
              var ce = _(v);
              v.set(ce, { reject: X, resolve: Y }), V === null ? O.postMessage({ id: ce, method: G2 }, te) : O.postMessage({ id: ce, method: G2, params: V }, te);
            });
          }, F = function(G2, V) {
            var te = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : [];
            O.postMessage({ id: null, method: G2, params: V }, te);
          }, U = {}, Q = 0, W = Object.entries(y); Q < W.length; Q++) {
            var H = n(W[Q], 2), z = H[0], J = H[1];
            U = a(a({}, U), {}, r({}, z, J({ call: M2, notify: F })));
          }
          return a({}, U);
        };
      };
          ...
          return i(s.mark(function y() {
            ...
            return s.wrap(function(U) {
              for (; ; ) switch (U.prev = U.next) {
                case 0:
                  return O = new MessageChannel(), v = O.port1, M2 = O.port2, U.next = 1, T("connect", { port: v }, [v]);
                case 1:
                  return F = U.sent, b.set(M2, F), U.abrupt("return", M2);
                case 2:
                case "end":
                  return U.stop();
              }
            ...
            var y = i(s.mark(function O(v) {
              ...
              return s.wrap(function(F) {
                for (; ; ) switch (F.prev = F.next) {
                  case 0:
                    if (M2 = b.get(v), M2 !== void 0) {
                      F.next = 1;
                      break;
                    }
                    throw new Error("The given port is not connected.");
                  case 1:
                    return F.next = 2, T("disconnect", { portId: M2 });
                  case 2:
                  case "end":
                    return F.stop();
                }
    ...
    }, P2 = c(f(/* @__PURE__ */ new WeakMap()), m(/* @__PURE__ */ new WeakMap()), e.generateUniqueNumber, g);
    t.createBroker = P2;
  }));
});
var Eg = j((Vo, Sg) => {
  ...
  (function(t, e) {
    ...
    var i = function(w) {
      ...
      return function(R) {
        return function(T) {
          for (var y = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : 0, O = arguments.length, v = new Array(O > 2 ? O - 2 : 0), M2 = 2; M2 < O; M2++) v[M2 - 2] = arguments[M2];
    ...
    }, c = /* @__PURE__ */ new Map([[0, null]]), f = /* @__PURE__ */ new Map([[0, null]]), h = i(c), d = s(f), m = o(r.generateUniqueNumber, c), g = a(r.generateUniqueNumber, f), P2 = e.createBroker({ clearInterval: function(w) {
      var _ = w.call;
      return h(function(R) {
...
var Ig = j((Go, Ag) => {
  ...
  (function(t, e) {
    ...
    var r = function(h, d) {
      ...
      return function() {
        if (m !== null) return m;
        var g = new Blob([d], { type: "application/javascript; charset=utf-8" }), P2 = URL.createObjectURL(g);
        return m = h(P2), setTimeout(function() {
...
var yi = j((Fr) => {
  ...
  var BA = () => {
    var _a;
    return typeof window < "u" ? typeof navigator < "u" && ((_a = navigator.userAgent) == null ? void 0 : _a.toLowerCase().indexOf(" electron/")) > -1 && (N2 == null ? void 0 : N2.versions) ? !Object.prototype.hasOwnProperty.call(N2.versions, "electron") : typeof window.document < "u" : false;
  }, xg = () => {
    var _a, _b2;
  ...
  var Fu = fr(ay()), $A = or(), Cg = fr(fy()), HA = fr(At()), Bg = qg(Na()), vn = fr(Ua()), VA = fr(Qy()), GA = fr(Iu()), zA = fr(Yy()), bi = Mr(), KA = Jy(), QA = fr(ju()), Mg = qg(yi()), Wu = globalThis.setImmediate || ((...t) => {
    ...
    publish(e, r, n, i) {
      ...
      let m = () => {
        ...
        let P2 = { cmd: "publish", topic: e, payload: r, qos: a, retain: c, messageId: g, dup: f };
        switch (s.protocolVersion === 5 && (P2.properties = h), this.log("publish :: qos", a), a) {
          case 1:
          case 2:
            this.outgoing[P2.messageId] = { volatile: false, cb: i || this.noop }, this.log("MqttClient:publish: packet cmd: %s", P2.cmd), this._sendPacket(P2, void 0, d);
            break;
          default:
            this.log("MqttClient:publish: packet cmd: %s", P2.cmd), this._sendPacket(P2, i, d);
            break;
        }
        return true;
      };
    ...
    subscribe(e, r, n) {
      ...
      let { properties: f } = r, h = [], d = (P2, S) => {
        if (S = S || r, !Object.prototype.hasOwnProperty.call(this._resubscribeTopics, P2) || this._resubscribeTopics[P2].qos < S.qos || s) {
          let b = { topic: P2, qos: S.qos };
      ...
      if (Array.isArray(e) ? e.forEach((P2) => {
        this.log("subscribe: array topic %s", P2), d(P2);
      }) : Object.keys(e).forEach((P2) => {
        this.log("subscribe: object topic %s, %o", P2, e[P2]), d(P2, e[P2]);
      }), !h.length) return n(null, []), this;
      let m = (P2, S) => {
        let b = { cmd: "subscribe", subscriptions: P2, messageId: S };
        ...
        let w = new Promise((_, R) => {
          this.outgoing[b.messageId] = { volatile: true, cb(T, y) {
            if (!T) {
              let { granted: O } = y;
              for (let v = 0; v < O.length; v += 1) P2[v].qos = O[v];
            }
        ...
        let P2 = this.options.subscribeBatchSize ?? h.length, S = [];
        for (let b = 0; b < h.length; b += P2) {
          let w = h.slice(b, b + P2), _ = this._nextId();
...
function JA() {
  if (Ug) return Gu;
  Ug = true;
  let t = 2147483647, e = 36, r = 1, n = 26, i = 38, s = 700, o = 72, a = 128, c = "-", f = /^xn--/, h = /[^\0-\x7F]/, d = /[\x2E\u3002\uFF0E\uFF61]/g, m = { overflow: "Overflow: input needs wider integers to process", "not-basic": "Illegal input >= 0x80 (not a basic code point)", "invalid-input": "Invalid input" }, g = e - r, P2 = Math.floor, S = String.fromCharCode;
  function b(H) {
  ...
  function _(H, z) {
    ...
    let G2 = H.split("."), V = w(G2, z).join(".");
    return ee + V;
  }
  function R(H) {
    let z = [], J = 0, ee = H.length;
    for (; J < ee; ) {
      let G2 = H.charCodeAt(J++);
      if (G2 >= 55296 && G2 <= 56319 && J < ee) {
        let V = H.charCodeAt(J++);
        (V & 64512) == 56320 ? z.push(((G2 & 1023) << 10) + (V & 1023) + 65536) : (z.push(G2), J--);
      } else z.push(G2);
    }
    return z;
  }
  let T = (H) => String.fromCodePoint(...H), y = function(H) {
    ...
    let ee = 0;
    for (H = J ? P2(H / s) : H >> 1, H += P2(H / z); H > g * n >> 1; ee += e) H = P2(H / g);
    return P2(ee + (g + 1) * H / (H + i));
  }, M2 = function(H) {
    let z = [], J = H.length, ee = 0, G2 = a, V = o, te = H.lastIndexOf(c);
    te < 0 && (te = 0);
    ...
    for (let Y = te > 0 ? te + 1 : 0; Y < J; ) {
      ...
      for (let Z = 1, ne = e; ; ne += e) {
        ...
        he >= e && b("invalid-input"), he > P2((t - ee) / Z) && b("overflow"), ee += he * Z;
        let be = ne <= V ? r : ne >= V + n ? n : ne - V;
        if (he < be) break;
        let ye = e - be;
        Z > P2(t / ye) && b("overflow"), Z *= ye;
      }
      let ce = z.length + 1;
      V = v(ee - X, ce, X == 0), P2(ee / ce) > t - G2 && b("overflow"), G2 += P2(ee / ce), ee %= ce, z.splice(ee++, 0, G2);
    }
    return String.fromCodePoint(...z);
  }, F = function(H) {
    let z = [];
    H = R(H);
    let J = H.length, ee = a, G2 = 0, V = o;
    ...
    for (te && z.push(c); Y < J; ) {
      ...
      let ce = Y + 1;
      X - ee > P2((t - G2) / ce) && b("overflow"), G2 += (X - ee) * ce, ee = X;
      for (let Z of H) if (Z < ee && ++G2 > t && b("overflow"), Z === ee) {
        let ne = G2;
        for (let he = e; ; he += e) {
          let be = he <= V ? r : he >= V + n ? n : he - V;
          if (ne < be) break;
          let ye = ne - be, Ae = e - be;
          z.push(S(O(be + ye % Ae, 0))), ne = P2(ye / Ae);
        }
        z.push(S(O(ne, 0))), V = v(G2, ce, Y === te), G2 = 0, ++Y;
      }
      ++G2, ++ee;
    }
    return z.join("");
  };
  return Gu = { version: "2.3.1", ucs2: { decode: R, encode: T }, decode: M2, encode: F, toASCII: function(H) {
    return _(H, function(z) {
      return h.test(z) ? "xn--" + F(z) : z;
    });
  }, toUnicode: function(H) {
    return _(H, function(z) {
      return f.test(z) ? M2(z.slice(4).toLowerCase()) : z;
    });
  } }, Gu;
}
var Gu;
...
function sI() {
  ...
  return nl = function(c) {
    ...
    }, g = r(0, f.length - h.length), P2 = [], S = 0; S < g; S++) P2[S] = "$" + S;
    if (d = Function("binder", "return function (" + o(P2, ",") + "){ return binder.apply(this,arguments); }")(m), f.prototype) {
      var b = function() {
      };
...
function Wr() {
  if (Zg) return sl;
  Zg = true;
  var t, e = ZA(), r = eI(), n = tI(), i = rI(), s = ab(), o = Sn(), a = nI(), c = Function, f = function(G2) {
    try {
      return c('"use strict"; return (' + G2 + ").constructor;")();
    } catch {
    }
  ...
  })() : d, g = iI()(), P2 = oI()(), S = Object.getPrototypeOf || (P2 ? function(G2) {
    return G2.__proto__;
...
function xx() {
  ...
  var i = { resolve: function() {
    ...
    for (var m = a.length, g = m - d, P2 = h < g ? h : g, S = -1, b = 0; b <= P2; ++b) {
      if (b === P2) {
        if (g > P2) {
          if (a.charCodeAt(d + b) === 47) return a.slice(d + b + 1);
          if (b === 0) return a.slice(d + b);
        } else h > P2 && (o.charCodeAt(c + b) === 47 ? S = b : b === 0 && (S = 0));
        break;
      }
    ...
    if (a !== void 0 && a.length > 0 && a.length <= o.length) {
      ...
      for (d = o.length - 1; d >= 0; --d) {
        var P2 = o.charCodeAt(d);
        if (P2 === 47) {
          if (!h) {
            c = d + 1;
            break;
          }
        } else g === -1 && (h = false, g = d + 1), m >= 0 && (P2 === a.charCodeAt(m) ? --m === -1 && (f = d) : (m = -1, f = g));
      }
    ...
    f ? (a.root = "/", h = 1) : h = 0;
    for (var d = -1, m = 0, g = -1, P2 = true, S = o.length - 1, b = 0; S >= h; --S) {
      ...
      g === -1 && (P2 = false, g = S + 1), c === 46 ? d === -1 ? d = S : b !== 1 && (b = 1) : d !== -1 && (b = -1);
    }
...
function Px() {
  ...
  Al = function k(B, re, ie, oe) {
    ...
    if (typeof B == "function" && !be(B)) {
      var Zl = Ct(B), ec = $(B, ut);
      return "[Function" + (Zl ? ": " + Zl : " (anonymous)") + "]" + (ec.length > 0 ? " { " + M2.call(ec, ", ") + " }" : "");
    }
    if (Oe(B)) {
    ...
    if (ne(B)) {
      ...
      var rs = $(B, ut);
      return Gr && !E(rs) ? "[" + q(rs, Gr) + "]" : "[ " + M2.call(rs, ", ") + " ]";
    }
    if (ye(B)) {
      var ns = $(B, ut);
      return !("cause" in Error.prototype) && "cause" in B && !ee.call(B, "cause") ? "{ [" + String(B) + "] " + M2.call(v.call("[cause]: " + ut(B.cause), ns), ", ") + " }" : ns.length === 0 ? "[" + String(B) + "]" : "{ [" + String(B) + "] " + M2.call(ns, ", ") + " }";
    }
    ...
    if (!he(B) && !be(B)) {
      var is = $(B, ut), ic = G2 ? G2(B) === Object.prototype : B instanceof Object || B.constructor === Object, os = B instanceof Object ? "" : "null prototype", oc = !ic && J && Object(B) === B && J in B ? _.call(Ne(B), 8, -1) : os ? "Object" : "", Kb = ic || typeof B.constructor != "function" ? "" : B.constructor.name ? B.constructor.name + " " : "", ss = Kb + (oc || os ? "[" + M2.call(v.call([], oc || [], os || []), ": ") + "] " : "");
      return is.length === 0 ? ss + "{}" : Gr ? ss + "{" + q(is, Gr) + "}" : ss + "{ " + M2.call(is, ", ") + " }";
    }
    return String(B);
  };
  function ce(k, B, re) {
...
`) >= 0) return false;
  ...
  function C(k, B) {
    var re;
    if (k.indent === "	") re = "	";
    else if (typeof k.indent == "number" && k.indent > 0) re = M2.call(Array(k.indent + 1), " ");
    else return null;
    return { base: re, prev: M2.call(Array(B + 1), re) };
  }
  function q(k, B) {
    if (k.length === 0) return "";
    var re = `
` + B.prev + B.base;
    return re + M2.call(k, "," + re) + `
` + B.prev;
  }
  function $(k, B) {
    var re = ne(k), ie = [];
...
function Ox() {
  ...
  return xl = function() {
    var w, _, R, T = { assert: function(y) {
      ...
      i && y && (typeof y == "object" || typeof y == "function") ? (w || (w = new i()), a(w, y, O)) : s ? (_ || (_ = new s()), h(_, y, O)) : (R || (R = { key: {}, next: null }), P2(R, y, O));
    } };
    return T;
  }, xl;
}
...
function Eb() {
  ...
  var t = Bl(), e = Object.prototype.hasOwnProperty, r = Array.isArray, n = (function() {
    ...
    for (var v = "", M2 = 0; M2 < O.length; M2 += f) {
      for (var F = O.length >= f ? O.slice(M2, M2 + f) : O, U = [], Q = 0; Q < F.length; ++Q) {
    ...
    for (var _ = [{ obj: { o: w }, prop: "o" }], R = [], T = 0; T < _.length; ++T) for (var y = _[T], O = y.obj[y.prop], v = Object.keys(O), M2 = 0; M2 < v.length; ++M2) {
      var F = v[M2], U = O[F];
  ...
  return Pl = { arrayToObject: s, assign: a, combine: P2, compact: d, decode: c, encode: h, isBuffer: g, isRegExp: m, maybeMap: S, merge: o }, Pl;
}
function Rx() {
  if (mb) return Ol;
  mb = true;
  var t = Ox(), e = Eb(), r = Bl(), n = Object.prototype.hasOwnProperty, i = { brackets: function(b) {
  ...
  }, m = {}, g = function S(b, w, _, R, T, y, O, v, M2, F, U, Q, W, H, z, J, ee, G2) {
    for (var V = b, te = G2, Y = 0, X = false; (te = te.get(m)) !== void 0 && !X; ) {
      var ce = te.get(b);
      if (Y += 1, typeof ce < "u") {
    ...
    if (typeof F == "function" ? V = F(w, V) : V instanceof Date ? V = W(V) : _ === "comma" && s(V) && (V = e.maybeMap(V, function(Ne) {
      ...
      if (y) return M2 && !J ? M2(w, h.encoder, ee, "key", H) : w;
      V = "";
    }
    if (d(V) || e.isBuffer(V)) {
      if (M2) {
        var Z = J ? w : M2(w, h.encoder, ee, "key", H);
        return [z(Z) + "=" + z(M2(V, h.encoder, ee, "value", H))];
      }
    ...
    var he;
    if (_ === "comma" && s(V)) J && M2 && (V = e.maybeMap(V, M2)), he = [{ value: V.length > 0 ? V.join(",") || null : void 0 }];
    else if (s(F)) he = F;
    else {
    ...
    for (var _e = 0; _e < he.length; ++_e) {
      ...
      if (!(O && Oe === null)) {
        ...
        G2.set(b, Y);
        var Re = t();
        Re.set(m, G2), a(ne, S(Oe, st, _, R, T, y, O, v, _ === "comma" && J && s(V) ? null : M2, F, U, Q, W, H, z, J, ee, Re));
      }
    }
    return ne;
  }, P2 = function(b) {
  ...
  return Ol = function(S, b) {
    ...
    for (var M2 = t(), F = 0; F < R.length; ++F) {
      var U = R[F];
      _.skipNulls && w[U] === null || a(y, g(w[U], U, O, v, _.allowEmptyArrays, _.strictNullHandling, _.skipNulls, _.encodeDotInKeys, _.encode ? _.encoder : null, _.filter, _.sort, _.allowDots, _.serializeDate, _.format, _.formatter, _.encodeValuesOnly, _.charset, M2));
    }
...
function kx() {
  ...
  var t = Eb(), e = Object.prototype.hasOwnProperty, r = Array.isArray, n = { allowDots: false, allowEmptyArrays: false, allowPrototypes: false, allowSparse: false, arrayLimit: 20, charset: "utf-8", charsetSentinel: false, comma: false, decodeDotInKeys: false, decoder: t.decode, delimiter: "&", depth: 5, duplicates: "combine", ignoreQueryPrefix: false, interpretNumericEntities: false, parameterLimit: 1e3, parseArrays: true, plainObjects: false, strictDepth: false, strictNullHandling: false }, i = function(m) {
    return m.replace(/&#(\d+);/g, function(g, P2) {
      return String.fromCharCode(parseInt(P2, 10));
    });
  }, s = function(m, g) {
    return m && typeof m == "string" && g.comma && m.indexOf(",") > -1 ? m.split(",") : m;
  }, o = "utf8=%26%2310003%3B", a = "utf8=%E2%9C%93", c = function(g, P2) {
    var S = { __proto__: null }, b = P2.ignoreQueryPrefix ? g.replace(/^\?/, "") : g;
    b = b.replace(/%5B/gi, "[").replace(/%5D/gi, "]");
    var w = P2.parameterLimit === 1 / 0 ? void 0 : P2.parameterLimit, _ = b.split(P2.delimiter, w), R = -1, T, y = P2.charset;
    if (P2.charsetSentinel) for (T = 0; T < _.length; ++T) _[T].indexOf("utf8=") === 0 && (_[T] === a ? y = "utf-8" : _[T] === o && (y = "iso-8859-1"), R = T, T = _.length);
    for (T = 0; T < _.length; ++T) if (T !== R) {
      var O = _[T], v = O.indexOf("]="), M2 = v === -1 ? O.indexOf("=") : v + 1, F, U;
      M2 === -1 ? (F = P2.decoder(O, n.decoder, y, "key"), U = P2.strictNullHandling ? null : "") : (F = P2.decoder(O.slice(0, M2), n.decoder, y, "key"), U = t.maybeMap(s(O.slice(M2 + 1), P2), function(W) {
        return P2.decoder(W, n.decoder, y, "value");
      })), U && P2.interpretNumericEntities && y === "iso-8859-1" && (U = i(U)), O.indexOf("[]=") > -1 && (U = r(U) ? [U] : U);
      var Q = e.call(S, F);
      Q && P2.duplicates === "combine" ? S[F] = t.combine(S[F], U) : (!Q || P2.duplicates === "last") && (S[F] = U);
    }
    return S;
  }, f = function(m, g, P2, S) {
    for (var b = S ? g : s(g, P2), w = m.length - 1; w >= 0; --w) {
      var _, R = m[w];
      if (R === "[]" && P2.parseArrays) _ = P2.allowEmptyArrays && (b === "" || P2.strictNullHandling && b === null) ? [] : [].concat(b);
      else {
        _ = P2.plainObjects ? /* @__PURE__ */ Object.create(null) : {};
        var T = R.charAt(0) === "[" && R.charAt(R.length - 1) === "]" ? R.slice(1, -1) : R, y = P2.decodeDotInKeys ? T.replace(/%2E/g, ".") : T, O = parseInt(y, 10);
        !P2.parseArrays && y === "" ? _ = { 0: b } : !isNaN(O) && R !== y && String(O) === y && O >= 0 && P2.parseArrays && O <= P2.arrayLimit ? (_ = [], _[O] = b) : y !== "__proto__" && (_[y] = b);
      }
      b = _;
    }
    return b;
    ...
    var P2 = typeof g.charset > "u" ? n.charset : g.charset, S = typeof g.duplicates > "u" ? n.duplicates : g.duplicates;
...
`, "	"], o = ["{", "}", "|", "\\", "^", "`"].concat(s), a = ["'"].concat(o), c = ["%", "/", "?", ";", "#"].concat(a), f = ["/", "?", "#"], h = 255, d = /^[+a-z0-9A-Z_-]{0,63}$/, m = /^([+a-z0-9A-Z_-]{0,63})(.*)$/, g = { javascript: true, "javascript:": true }, P2 = { javascript: true, "javascript:": true }, S = { http: true, https: true, ftp: true, gopher: true, file: true, "http:": true, "https:": true, "ftp:": true, "gopher:": true, "file:": true }, b = Cx();
  function w(y, O, v) {
    if (y && typeof y == "object" && y instanceof e) return y;
    var M2 = new e();
    return M2.parse(y, O, v), M2;
  }
  e.prototype.parse = function(y, O, v) {
    if (typeof y != "string") throw new TypeError("Parameter 'url' must be a string, not " + typeof y);
    var M2 = y.indexOf("?"), F = M2 !== -1 && M2 < y.indexOf("#") ? "?" : "#", U = y.split(F), Q = /\\/g;
    ...
    if (v || z || W.match(/^\/\/[^@/]+@[^@/]+/)) {
      var ee = W.substr(0, 2) === "//";
      ee && !(z && P2[z]) && (W = W.substr(2), this.slashes = true);
    }
    if (!P2[z] && (ee || z && !S[z])) {
      for (var G2 = -1, V = 0; V < f.length; V++) {
        var te = W.indexOf(f[V]);
        te !== -1 && (G2 === -1 || te < G2) && (G2 = te);
      }
      var Y, X;
      G2 === -1 ? X = W.lastIndexOf("@") : X = W.lastIndexOf("@", G2), X !== -1 && (Y = W.slice(0, X), W = W.slice(X + 1), this.auth = decodeURIComponent(Y)), G2 = -1;
      for (var V = 0; V < c.length; V++) {
        var te = W.indexOf(c[V]);
        te !== -1 && (G2 === -1 || te < G2) && (G2 = te);
      }
      G2 === -1 && (G2 = W.length), this.host = W.slice(0, G2), W = W.slice(G2), this.parseHost(), this.hostname = this.hostname || "";
  ...
  e.prototype.format = function() {
    ...
    var O = this.protocol || "", v = this.pathname || "", M2 = this.hash || "", F = false, U = "";
    ...
    return O && O.substr(-1) !== ":" && (O += ":"), this.slashes || (!O || S[O]) && F !== false ? (F = "//" + (F || ""), v && v.charAt(0) !== "/" && (v = "/" + v)) : F || (F = ""), M2 && M2.charAt(0) !== "#" && (M2 = "#" + M2), Q && Q.charAt(0) !== "?" && (Q = "?" + Q), v = v.replace(/[?#]/g, function(W) {
      return encodeURIComponent(W);
    }), Q = Q.replace("#", "%23"), O + F + v + Q + M2;
  };
  function R(y, O) {
  ...
  return e.prototype.resolveObject = function(y) {
    ...
    for (var v = new e(), M2 = Object.keys(this), F = 0; F < M2.length; F++) {
      var U = M2[F];
      v[U] = this[U];
    }
    ...
    if (y.protocol && y.protocol !== v.protocol) {
      ...
      if (v.protocol = y.protocol, !y.host && !P2[y.protocol]) {
...
function Ib(t) {
  ...
  let n = new URL("file://");
  return e.includes("%") && (e = e.replace(Gx2, "%25")), !Cl && e.includes("\\") && (e = e.replace(zx, "%5C")), e.includes(`
...
var Tb = ze(() => {
  ...
  Ge.pathToFileURL = Ib;
  Ge.fileURLToPath = Ab;
  Nx = Ge.Url, qx = Ge.format, Lx = Ge.resolve, Ux = Ge.resolveObject, Dx = Ge.parse, jx = Ge.URL, Fx = 92, Wx = 47, $x = 97, Hx = 122, Cl = Mx === "win32", Vx = /\//g, Gx2 = /%/g, zx = /\\/g, Kx = /\n/g, Qx = /\r/g, Yx = /\t/g;
});
var Ob = j((I9, Pb) => {
  "use strict";
  A();
...
var _i = j((gr) => {
  ...
  var uT = (t, e) => {
    ...
    a.readyState === a.OPEN ? (r = c, r.socket = a) : (r = new Nl.BufferedDuplex(e, c, a), f ? a.addEventListener("open", d) : a.onopen = d), f ? (a.addEventListener("close", m), a.addEventListener("error", g), a.addEventListener("message", P2)) : (a.onclose = m, a.onerror = g, a.onmessage = P2);
    function h(w, _, R) {
...
var $b = j((In) => {
  ...
  typeof (N2 == null ? void 0 : N2.nextTick) != "function" && (N2.nextTick = setImmediate);
...
var { floor: floor3, min: min3, sin: sin3 } = Math;
var libName3 = "Trystero";
var alloc3 = (n, f) => Array(n).fill(void 0).map(f);
var charSet3 = "0123456789AaBbCcDdEeFfGgHhIiJjKkLlMmNnOoPpQqRrSsTtUuVvWwXxYyZz";
var genId3 = (n) => alloc3(n, () => charSet3[floor3(Math.random() * 62)] ?? "").join("");
var selfId3 = genId3(20);
var all3 = Promise.all.bind(Promise);
var isBrowser3 = typeof window !== "undefined";
var { entries: entries3, fromEntries: fromEntries3, keys: keys3, values: values3 } = Object;
var noOp3 = () => {
};
var candidateType3 = "candidate";
var resetTimer3 = (timer) => {
  if (timer !== null) clearTimeout(timer);
  return null;
};
var mkErr3 = (msg) => /* @__PURE__ */ new Error(`${libName3}: ${msg}`);
var toErrorMessage3 = (reason, fallback) => {
  if (reason instanceof Error && reason.message) return reason.message;
  if (typeof reason === "string" && reason) return reason;
  return toJson3(reason ?? fallback);
};
var toError3 = (reason, fallback) => reason instanceof Error ? reason : mkErr3(toErrorMessage3(reason, fallback));
var encoder3 = new TextEncoder();
var decoder3 = new TextDecoder();
var encodeBytes3 = (txt) => encoder3.encode(txt);
var decodeBytes3 = (buffer) => decoder3.decode(buffer);
var toHex3 = (buffer) => buffer.reduce((a, c) => a + c.toString(16).padStart(2, "0"), "");
var topicPath3 = (...parts) => parts.join("@");
var shuffle3 = (xs, seed) => {
  const a = [...xs];
  const rand = () => {
    const x2 = sin3(seed++) * 1e4;
    return x2 - floor3(x2);
  };
  let i = a.length;
  while (i) {
    const j2 = floor3(rand() * i--);
    const tmp = a[i];
    a[i] = a[j2];
    a[j2] = tmp;
  }
...
var getRelays3 = (config, defaults, defaultN, deriveFromAppId = false) => {
  var _a, _b2;
  return ((_a = config.relayConfig) == null ? void 0 : _a.urls) || (deriveFromAppId ? shuffle3(defaults, strToNum3(config.appId)) : defaults).slice(0, ((_b2 = config.relayConfig) == null ? void 0 : _b2.redundancy) ?? defaultN);
};
var toJson3 = JSON.stringify;
var fromJson3 = (s) => {
  try {
    return JSON.parse(s);
  } catch {
    throw mkErr3(`failed to parse JSON: ${s}`);
  }
};
var strToNum3 = (str, limit = Number.MAX_SAFE_INTEGER) => str.split("").reduce((a, c) => a + c.charCodeAt(0), 0) % limit;
var reconnectionLockingPromise3 = null;
var resolver3 = null;
var pauseRelayReconnection3 = () => {
  if (!reconnectionLockingPromise3) reconnectionLockingPromise3 = new Promise((resolve) => {
    resolver3 = resolve;
  }).finally(() => {
    resolver3 = null;
    reconnectionLockingPromise3 = null;
  });
};
var resumeRelayReconnection3 = () => {
  resolver3 == null ? void 0 : resolver3();
};
var createRelayManager3 = (getSocket) => {
  const relays = {};
  const keysByRelay = /* @__PURE__ */ new WeakMap();
  const keyOf = (relay) => {
    const key = keysByRelay.get(relay);
    if (!key) throw mkErr3("relay bookkeeping missing registration for relay client");
    return key;
  };
  const scoped = () => {
    const store2 = {};
  ...
  return {
    ...
    keyOf,
    scoped,
    getSockets: () => fromEntries3(entries3(relays).flatMap(([key, relay]) => {
      const socket = getSocket(relay);
      return socket ? [[key, socket]] : [];
    }))
  };
...
var watchOnline3 = () => {
  if (isBrowser3) {
    const controller = new AbortController();
    addEventListener("online", resumeRelayReconnection3, { signal: controller.signal });
    addEventListener("offline", pauseRelayReconnection3, { signal: controller.signal });
    return () => controller.abort();
  }
  return noOp3;
};
...
var algo3 = "AES-GCM";
var strToSha13 = {};
var pack3 = (buff) => btoa(String.fromCharCode.apply(null, Array.from(new Uint8Array(buff))));
var unpack3 = (packed) => {
  const str = atob(packed);
  return new Uint8Array(str.length).map((_, i) => str.charCodeAt(i)).buffer;
};
var hashWith3 = async (algorithm, str) => new Uint8Array(await crypto.subtle.digest(algorithm, encodeBytes3(str)));
var sha13 = async (str) => strToSha13[str] ?? (strToSha13[str] = Array.from(await hashWith3("SHA-1", str)).map((b) => b.toString(36)).join(""));
var genKey3 = async (secret, appId, roomId) => crypto.subtle.importKey("raw", await crypto.subtle.digest({ name: "SHA-256" }, encodeBytes3(`${secret}:${appId}:${roomId}`)), { name: algo3 }, false, ["encrypt", "decrypt"]);
var deriveRoomNamespace3 = async (appId, roomId) => toHex3(await hashWith3("SHA-256", `${libName3}:${appId}:${roomId}`));
var joinChar3 = "$";
var ivJoinChar3 = ",";
var encrypt3 = async (keyP, plaintext) => {
  const iv = crypto.getRandomValues(/* @__PURE__ */ new Uint8Array(16));
  return iv.join(ivJoinChar3) + joinChar3 + pack3(await crypto.subtle.encrypt({
    name: algo3,
    iv
  }, await keyP, encodeBytes3(plaintext)));
};
var decrypt3 = async (keyP, raw) => {
  const [iv, c] = raw.split(joinChar3);
  return decodeBytes3(await crypto.subtle.decrypt({
    name: algo3,
    iv: new Uint8Array((iv == null ? void 0 : iv.split(ivJoinChar3).map(Number)) ?? [])
  }, await keyP, unpack3(c ?? "")));
};

// ../../../../node_modules/@trystero-p2p/mqtt/node_modules/@trystero-p2p/core/dist/offer-pool.mjs
var offerTtl3 = 57333;
var offerLeaseTtlMs3 = 18e4;
var poolSize3 = 20;
var OfferPool3 = class {
  constructor(makeOffer) {
    __publicField(this, "makeOffer");
    __publicField(this, "pool", []);
    __publicField(this, "pooled", /* @__PURE__ */ new Set());
  ...
  warmup() {
    this.pool = [];
    this.pooled.clear();
    alloc3(poolSize3, this.makeOffer).forEach((p) => this.push(p));
    this.active = true;
    this.cleanupTimer = setInterval(() => {
      this.pool = this.pool.filter((peer) => {
        if (peer.isDead) {
  ...
  checkout(n, leaseOffers, encryptOffer) {
    const peers = this.shift(n);
    const missing = Math.max(0, n - peers.length);
    if (missing > 0) peers.push(...alloc3(missing, this.makeOffer));
    const toRecord = async (candidate, didRetry = false) => {
      try {
        const offer = await encryptOffer(candidate);
        if (leaseOffers) {
    ...
    return all3(peers.map((peer) => toRecord(peer)));
  }
  getOffers(n, encryptOffer) {
    return this.checkout(n, true, encryptOffer);
  }
...
var overlapRoomPasswordErr3 = mkErr3("incorrect password for overlapping room");
var createPasswordHandshake3 = (password, appId, roomId) => {
  const hashChallenge = (challenge2) => hashWith3("SHA-256", `${challenge2}:${password}:${appId}:${roomId}`).then(toHex3);
  const run = async (send2, receive, isInitiator) => {
    if (!password) return;
    if (isInitiator) {
      const challenge2 = genId3(36);
      await send2({
        __trystero_pw: "challenge",
        c: challenge2
      });
      const { data: data2 } = await receive();
      if (!data2 || typeof data2 !== "object" || data2.__trystero_pw !== "response" || typeof data2.h !== "string") throw overlapRoomPasswordErr3;
      const expected = await hashChallenge(challenge2);
      if (data2.h !== expected) throw overlapRoomPasswordErr3;
      return;
    }
    const { data } = await receive();
    if (!data || typeof data !== "object" || data.__trystero_pw !== "challenge" || typeof data.c !== "string") throw overlapRoomPasswordErr3;
    await send2({
      __trystero_pw: "response",
      h: await hashChallenge(data.c)
    });
...
var toHandshakeErrorMessage3 = (error) => {
  const message = toErrorMessage3(error, "unknown error");
  return message.startsWith("handshake ") ? message : `handshake failed: ${message}`;
};
var createHandshakeManager3 = ({ onPeerHandshake, onHandshakeError, handshakeTimeoutMs, sendHandshakeData, sendHandshakeReady, onActivate, onFailure }) => {
  const peerStates = {};
  const maybeActivatePeer = (id, peer) => {
    const state = peerStates[id];
    if (!state || peer && state.peer !== peer || state.isActive) return;
    if (!state.didLocalHandshakePass || !state.didReceiveRemoteReady) return;
    state.isActive = true;
    state.handshakeTimer = resetTimer3(state.handshakeTimer);
    onActivate(id, state.peer);
  };
  const failPeerHandshake = (id, peer, reason) => {
    const state = peerStates[id];
    if (!state || state.peer !== peer) return;
    const error = toHandshakeErrorMessage3(reason);
    onHandshakeError == null ? void 0 : onHandshakeError(id, error);
    onFailure(id, peer, mkErr3(error));
  };
  const markLocalHandshakePassed = (id, peer) => {
    const state = peerStates[id];
    if (!state || state.peer !== peer || state.isActive) return;
    state.didLocalHandshakePass = true;
    sendHandshakeReady("", id).catch((err) => failPeerHandshake(id, peer, mkErr3(`failed sending handshake readiness: ${toErrorMessage3(err, "unknown send failure")}`)));
    maybeActivatePeer(id, peer);
  };
  return {
    addPeer: (id, peer) => {
    ...
    start: (id, peer) => {
      const state = peerStates[id];
      if (!state || state.peer !== peer) return;
      state.handshakeTimer = setTimeout(() => failPeerHandshake(id, peer, mkErr3(`handshake timed out after ${handshakeTimeoutMs}ms`)), handshakeTimeoutMs);
      const sendHandshake = async (data, metadata) => {
        await sendHandshakeData(data, id, metadata);
      };
      const receiveHandshake = () => new Promise((resolve, reject) => {
        const current = peerStates[id];
        if (!current || current.peer !== peer) {
          reject(mkErr3("peer disconnected during handshake"));
          return;
        }
        const payload = current.pendingHandshakePayloads.shift();
        if (payload) {
      ...
      const isInitiator = selfId3 < id;
      Promise.resolve(onPeerHandshake == null ? void 0 : onPeerHandshake(id, sendHandshake, receiveHandshake, isInitiator)).then(() => markLocalHandshakePassed(id, peer)).catch((err) => failPeerHandshake(id, peer, toError3(err, "handshake failed")));
    },
    receiveHandshakeData: (data, id, metadata) => {
      const state = peerStates[id];
      if (!state || state.isActive) return;
...
var iceTimeout3 = 15e3;
var disconnectedCloseDelayMs3 = 5e3;
var iceStateEvent3 = "icegatheringstatechange";
var iceConnectionStateEvent3 = "iceconnectionstatechange";
var offerType3 = "offer";
var answerType3 = "answer";
var outOfRangePattern3 = /out of range/i;
var rewriteMdnsCandidatesToLoopback3 = (sdp) => sdp.replace(/ (\S+\.local) (\d+) typ host/g, " 127.0.0.1 $2 typ host");
var peer_default3 = (initiator, { trickleIce, rtcConfig, rtcPolyfill, turnConfig, _test_only_mdnsHostFallbackToLoopback }) => {
  const pc2 = new (rtcPolyfill ?? RTCPeerConnection)({
    iceServers: defaultIceServers3.concat(turnConfig ?? []),
    ...rtcConfig
  });
  const handlers = {};
  const pendingSignals = [];
  ...
  let disconnectedCloseTimer = null;
  let didEmitClose = false;
  const clearDisconnectedCloseTimer = () => disconnectedCloseTimer = resetTimer3(disconnectedCloseTimer);
  const emitClose = () => {
    var _a;
    if (didEmitClose) return;
    didEmitClose = true;
  ...
  const normalizeSdp = (sdp) => _test_only_mdnsHostFallbackToLoopback ? rewriteMdnsCandidatesToLoopback3(sdp) : sdp;
  const normalizeCandidate = (candidate) => {
    if (!_test_only_mdnsHostFallbackToLoopback || typeof candidate.candidate !== "string") return candidate;
    const normalizedCandidate = rewriteMdnsCandidatesToLoopback3(candidate.candidate);
    return normalizedCandidate === candidate.candidate ? candidate : {
      ...candidate,
      candidate: normalizedCandidate
    };
  ...
  const localDescriptionSignal = (peerConnection) => {
    var _a, _b2;
    return {
      type: ((_a = peerConnection.localDescription) == null ? void 0 : _a.type) ?? offerType3,
      sdp: normalizeSdp(((_b2 = peerConnection.localDescription) == null ? void 0 : _b2.sdp) ?? "")
    };
  };
  const getRemoteUfrag = () => {
  ...
  const addIceCandidateSafe = async (candidate) => {
    try {
      ...
      if (err instanceof Error && outOfRangePattern3.test(err.message) && typeof candidate.sdpMLineIndex === "number") return false;
      throw err;
    }
  };
  const flushPendingRemoteCandidates = async () => {
  ...
  const setupDataChannel = (channel) => {
    ...
    channel.onerror = ({ error }) => {
      var _a;
      return (_a = handlers.error) == null ? void 0 : _a.call(handlers, toError3(error, "data channel error"));
    };
  };
  const waitForIceGathering = async (peerConnection) => {
    let timeout = null;
    try {
      await Promise.race([new Promise((res) => {
        const checkState = () => {
          if (peerConnection.iceGatheringState === "complete") {
            peerConnection.removeEventListener(iceStateEvent3, checkState);
            res();
          }
        };
        peerConnection.addEventListener(iceStateEvent3, checkState);
        checkState();
      }), new Promise((res) => {
        timeout = setTimeout(res, iceTimeout3);
      })]);
    } finally {
      resetTimer3(timeout);
    }
  ...
  const createOffer = async (restartIce = false) => {
    ...
    try {
      makingOffer = true;
      if (restartIce) {
        if (pc2.signalingState !== "stable" && pc2.signalingState !== "closed" && ((_a = pc2.localDescription) == null ? void 0 : _a.type) === offerType3) await pc2.setLocalDescription({ type: "rollback" });
        if (typeof pc2.restartIce === "function") pc2.restartIce();
      }
      await pc2.setLocalDescription(restartIce ? await pc2.createOffer({ iceRestart: true }) : void 0);
      return await emitLocalDescriptionSignal();
    } catch (err) {
      (_b2 = handlers.error) == null ? void 0 : _b2.call(handlers, toError3(err, "failed to create local offer"));
    } finally {
      makingOffer = false;
    }
  };
  ...
  const handleConnectionStateChange = () => {
    ...
    if (pc2.connectionState === "disconnected" || pc2.iceConnectionState === "disconnected") {
      if (!disconnectedCloseTimer) disconnectedCloseTimer = setTimeout(() => {
        disconnectedCloseTimer = null;
        if (pc2.connectionState === "disconnected" || pc2.iceConnectionState === "disconnected") emitClose();
      }, disconnectedCloseDelayMs3);
      return;
    }
  };
  pc2.onconnectionstatechange = handleConnectionStateChange;
  pc2.addEventListener(iceConnectionStateEvent3, handleConnectionStateChange);
  pc2.ontrack = (e) => {
    var _a, _b2;
    const stream = e.streams[0];
    if (stream) {
  ...
  const offerPromise = initiator ? new Promise((res) => appendSignalHandler((signal) => {
    if (signal.type === offerType3) res(signal);
  })) : Promise.resolve();
  if (initiator) queueMicrotask(() => {
    var _a;
    if (!makingOffer && pc2.signalingState === "stable" && !pc2.localDescription && pc2.connectionState !== "closed") (_a = pc2.onnegotiationneeded) == null ? void 0 : _a.call(pc2, new Event("negotiationneeded"));
  ...
  return {
    ...
    getOffer: async (restartIce = false) => {
      ...
      if (!initiator) return;
      if (restartIce) return createOffer(true);
      if (((_a = pc2.localDescription) == null ? void 0 : _a.type) === offerType3) return shouldTrickleIce ? localDescriptionSignal(pc2) : waitForIceGathering(pc2);
      return offerPromise;
    },
    async signal(sdp) {
      var _a, _b2, _c2;
      if (sdp.type === "candidate") {
        try {
          ...
          (_a = handlers.error) == null ? void 0 : _a.call(handlers, toError3(err, "failed to parse remote candidate"));
        }
        return;
      }
      if ((dataChannel == null ? void 0 : dataChannel.readyState) === "open" && !((_b2 = sdp.sdp) == null ? void 0 : _b2.includes("a=rtpmap"))) return;
      try {
        ...
        if (sdp.type === offerType3) {
          if (makingOffer || pc2.signalingState !== "stable" && !isSettingRemoteAnswerPending) {
            if (initiator) return;
            await all3([pc2.setLocalDescription({ type: "rollback" }), pc2.setRemoteDescription(rtcSdp)]);
          } else await pc2.setRemoteDescription(rtcSdp);
          await flushPendingRemoteCandidates();
          await pc2.setLocalDescription();
          return await emitLocalDescriptionSignal();
        }
        if (sdp.type === answerType3) {
          isSettingRemoteAnswerPending = true;
          try {
            await pc2.setRemoteDescription(rtcSdp);
            await flushPendingRemoteCandidates();
        ...
        (_c2 = handlers.error) == null ? void 0 : _c2.call(handlers, toError3(err, "failed to apply remote signal"));
      }
    },
    sendData: (data) => dataChannel == null ? void 0 : dataChannel.send(data),
    destroy: () => {
...
var defaultIceServers3 = [...alloc3(3, (_, i) => `stun:stun${i || ""}.l.google.com:19302`), "stun:stun.cloudflare.com:3478"].map((url) => ({ urls: url }));

// ../../../../node_modules/@trystero-p2p/mqtt/node_modules/@trystero-p2p/core/dist/action-wire.mjs
var TypedArray3 = Object.getPrototypeOf(Uint8Array);
var typeByteLimit3 = 32;
var typeIndex3 = 0;
var nonceIndex3 = 32;
var tagIndex3 = 34;
var progressIndex3 = 35;
var payloadIndex3 = 36;
var chunkSize3 = 16 * 2 ** 10 - payloadIndex3;
var oneByteMax3 = 255;
var twoByteMax3 = 65535;
var buffLowEvent3 = "bufferedamountlow";
var channelCloseEvent3 = "close";
var channelErrorEvent3 = "error";
var backpressureWaitTimeoutMs3 = 1e4;
var toByteArray3 = (value) => value instanceof ArrayBuffer ? new Uint8Array(value) : new Uint8Array(value.buffer, value.byteOffset, value.byteLength);
var waitForBufferedAmountLow3 = (channel, timeoutMs = backpressureWaitTimeoutMs3) => {
  if (channel.readyState !== "open" || channel.bufferedAmount <= channel.bufferedAmountLowThreshold) return Promise.resolve(channel.readyState === "open");
  return new Promise((res) => {
    let settled = false;
    let timeout = null;
    const finish = (didDrain) => {
      if (settled) return;
      settled = true;
      channel.removeEventListener(buffLowEvent3, onBufferLow);
      channel.removeEventListener(channelCloseEvent3, onCloseOrError);
      channel.removeEventListener(channelErrorEvent3, onCloseOrError);
      resetTimer3(timeout);
      res(didDrain);
    };
    const onBufferLow = () => finish(true);
    const onCloseOrError = () => finish(false);
    channel.addEventListener(buffLowEvent3, onBufferLow);
    channel.addEventListener(channelCloseEvent3, onCloseOrError);
    channel.addEventListener(channelErrorEvent3, onCloseOrError);
    timeout = setTimeout(() => finish(false), timeoutMs);
    if (channel.readyState !== "open") {
      finish(false);
      return;
...
var createActionWireManager3 = ({ getPeer, getPeerIds, canReceiveFromPeer, throwIfAborted: throwIfAborted4 }) => {
  const actions = {};
  const actionsCache = {};
  const pendingTransmissions = {};
  const pendingActionPayloads = {};
  const iterate = (targets, f, { includePending = false } = {}) => (targets ? Array.isArray(targets) ? targets : [targets] : getPeerIds(includePending)).flatMap((id) => {
    const peer = getPeer(id, includePending);
    if (!peer) {
      console.warn(`${libName3}: no peer with id ${id} found`);
      return [];
    }
    return [Promise.resolve(f(id, peer))];
  });
  const makeInternalAction = (type, options = {}) => {
    ...
    if (actions[type] && cached) {
      const cachedOptions = actions[type].options;
      if (cachedOptions.sendToPending !== Boolean(options.sendToPending) || cachedOptions.receiveWhilePending !== Boolean(options.receiveWhilePending)) throw mkErr3(`action type "${type}" cannot be redefined`);
      return cached;
    }
    if (!type) throw mkErr3("action type argument is required");
    const typeBytes = encodeBytes3(type);
    if (typeBytes.byteLength > typeByteLimit3) throw mkErr3(`action type string "${type}" (${typeBytes.byteLength}b) exceeds byte limit (${typeByteLimit3}). Hint: choose a shorter name.`);
    const normalizedOptions = {
      sendToPending: Boolean(options.sendToPending),
      receiveWhilePending: Boolean(options.receiveWhilePending)
    };
    const typeBytesPadded = new Uint8Array(typeByteLimit3);
    typeBytesPadded.set(typeBytes);
    let nonce = 0;
    actions[type] = {
      onComplete: noOp3,
      ...
      send: async (data, targets, meta, onProgress, signal) => {
        throwIfAborted4(signal);
        const dataType = typeof data;
        if (dataType === "undefined") throw mkErr3("action data cannot be undefined");
        const isJson = dataType !== "string";
        const isBlob = data instanceof Blob;
        const isBinary = isBlob || data instanceof ArrayBuffer || data instanceof TypedArray3;
        const hasMeta = meta !== void 0;
        const buffer = isBinary ? toByteArray3(isBlob ? await data.arrayBuffer() : data) : encodeBytes3(isJson ? toJson3(data) : data);
        const metaEncoded = hasMeta ? encodeBytes3(toJson3(meta)) : null;
        const chunkTotal = Math.ceil(buffer.byteLength / chunkSize3) + (hasMeta ? 1 : 0) || 1;
        const chunks = alloc3(chunkTotal, (_, i) => {
          const isLast = i === chunkTotal - 1;
          const isMeta = Boolean(hasMeta && i === 0);
          const chunk = new Uint8Array(payloadIndex3 + (isMeta ? (metaEncoded == null ? void 0 : metaEncoded.byteLength) ?? 0 : isLast ? buffer.byteLength - chunkSize3 * (chunkTotal - (hasMeta ? 2 : 1)) : chunkSize3));
          chunk.set(typeBytesPadded);
          chunk.set([nonce >> 8, nonce & oneByteMax3], nonceIndex3);
          chunk.set([Number(isLast) | Number(isMeta) << 1 | Number(isBinary) << 2 | Number(isJson) << 3], tagIndex3);
          chunk.set([Math.round((i + 1) / chunkTotal * oneByteMax3)], progressIndex3);
          chunk.set(hasMeta ? isMeta ? metaEncoded ?? /* @__PURE__ */ new Uint8Array() : buffer.subarray((i - 1) * chunkSize3, i * chunkSize3) : buffer.subarray(i * chunkSize3, (i + 1) * chunkSize3), payloadIndex3);
          return chunk;
        });
        nonce = nonce + 1 & twoByteMax3;
        await all3(iterate(targets, async (id, peer) => {
          const { channel } = peer;
          let chunkN = 0;
          while (chunkN < chunkTotal) {
            throwIfAborted4(signal);
            ...
            if (!chunk) break;
            if (channel && channel.bufferedAmount > channel.bufferedAmountLowThreshold) {
              const didDrain = await waitForBufferedAmountLow3(channel);
              throwIfAborted4(signal);
              if (!didDrain) break;
            }
            const currentPeer = getPeer(id, normalizedOptions.sendToPending);
            ...
            peer.sendData(chunk);
            chunkN++;
            const progressByte = chunk[progressIndex3] ?? oneByteMax3;
            onProgress == null ? void 0 : onProgress(progressByte / oneByteMax3, id, meta);
          }
        }, { includePending: normalizedOptions.sendToPending }));
        return [];
      },
  ...
  const handleData = (id, data) => {
    var _a, _b2;
    const buffer = new Uint8Array(data);
    const type = decodeBytes3(buffer.subarray(typeIndex3, nonceIndex3)).replaceAll("\0", "");
    const action = actions[type];
    if (!canReceiveFromPeer(id, Boolean(action == null ? void 0 : action.options.receiveWhilePending))) return;
    const nonce = (buffer[nonceIndex3] ?? 0) << 8 | (buffer[33] ?? 0);
    const tag2 = buffer[tagIndex3] ?? 0;
    const progress = buffer[progressIndex3] ?? 0;
    const payload = buffer.subarray(payloadIndex3);
    const isLast = Boolean(tag2 & 1);
    const isMeta = Boolean(tag2 & 2);
    const isBinary = Boolean(tag2 & 4);
    const isJson = Boolean(tag2 & 8);
    ...
    (_a = pendingTransmissions[id])[type] ?? (_a[type] = {});
    const target = (_b2 = pendingTransmissions[id][type])[nonce] ?? (_b2[nonce] = { chunks: [] });
    if (isMeta) target.meta = fromJson3(decodeBytes3(payload));
    else target.chunks.push(payload);
    action == null ? void 0 : action.onProgress(progress / oneByteMax3, id, target.meta);
    if (!isLast) return;
    const full = new Uint8Array(target.chunks.reduce((a, c) => a + c.byteLength, 0));
    target.chunks.reduce((a, c) => {
      full.set(c, a);
    ...
    delete pendingTransmissions[id][type][nonce];
    const payloadValue = isBinary ? full : isJson ? fromJson3(decodeBytes3(full)) : decodeBytes3(full);
    if (action) {
      action.onComplete(payloadValue, id, target.meta);
      return;
    }
...
var requestHandlerBufferMs3 = 500;
var makeActionError3 = (kind, message) => {
  const error = mkErr3(message);
  error.kind = kind;
  error.name = kind === "aborted" ? "AbortError" : error.name;
  return error;
};
var throwIfAborted3 = (signal) => {
  if (signal == null ? void 0 : signal.aborted) throw makeActionError3("aborted", "operation aborted");
};
var getRequestMetadata3 = (metadata) => {
  if (metadata && typeof metadata === "object" && !Array.isArray(metadata) && typeof metadata.r === "string") return {
    r: metadata.r,
    ...Object.hasOwn(metadata, "m") ? { m: metadata.m } : {}
  };
...
var getResponseMetadata3 = (metadata) => {
  if (metadata && typeof metadata === "object" && !Array.isArray(metadata) && typeof metadata.r === "string") return {
    r: metadata.r,
    ...typeof metadata.e === "string" ? { e: metadata.e } : {}
  };
...
var withMetadata3 = (context, metadata) => metadata === void 0 ? context : {
  ...context,
  metadata
};
var createActionManager3 = ({ getPeer, getPeerIds, canReceiveFromPeer }) => {
  const publicActions = {};
  const pendingRequestWaiters = {};
  const wire = createActionWireManager3({
    getPeer,
    getPeerIds,
    canReceiveFromPeer,
    throwIfAborted: throwIfAborted3
  });
  const makeInternalAction = wire.makeInternalAction;
  const handleData = wire.handleData;
  const clearPendingRequestWaiter = (requestId) => {
    const waiter = pendingRequestWaiters[requestId];
    if (!waiter) return;
    resetTimer3(waiter.timer);
    if (waiter.signal && waiter.abortHandler) waiter.signal.removeEventListener("abort", waiter.abortHandler);
    delete pendingRequestWaiters[requestId];
  };
  const rejectPendingRequestsForPeer = (id, error) => {
    entries3(pendingRequestWaiters).forEach(([requestId, waiter]) => {
      if (waiter.peerId !== id) return;
      clearPendingRequestWaiter(requestId);
      waiter.reject(error);
    });
  ...
  const clearPeer = (id, error) => {
    wire.clearPeer(id);
    rejectPendingRequestsForPeer(id, makeActionError3("disconnected", toErrorMessage3(error, "peer disconnected")));
  };
  const responseAction = makeInternalAction("@_response");
  responseAction.onMessage((payload, id, metadata) => {
    const parsed = getResponseMetadata3(metadata);
    if (!parsed) return;
    const waiter = pendingRequestWaiters[parsed.r];
    if (!waiter || waiter.peerId !== id) return;
    clearPendingRequestWaiter(parsed.r);
    if (parsed.e !== void 0) {
      waiter.reject(makeActionError3("rejected", parsed.e));
      return;
    }
    waiter.resolve(payload);
  });
  const makeActionImpl = (type, config) => {
    if (config && "onRequest" in config && config.kind !== "request") throw mkErr3('request actions must use kind: "request"');
    const kind = (config == null ? void 0 : config.kind) ?? "message";
    const rawAction = makeInternalAction(type);
    const existingState = publicActions[type];
    if (existingState) {
      if (existingState.kind !== kind) throw mkErr3(`action type "${type}" cannot be redefined`);
      return existingState.action;
    }
    const state = {
      kind,
    ...
    const toProgressHandler = (handler, metadata) => handler ? (progress, peerId) => handler(progress, withMetadata3({ peerId }, metadata)) : void 0;
    const setReceiveProgress = (handler) => {
      state.onReceiveProgress = handler;
    };
    const dispatchReceiveProgress = (progress, peerId, metadata) => {
      var _a;
      const requestMetadata = state.kind === "request" ? getRequestMetadata3(metadata) : null;
      (_a = state.onReceiveProgress) == null ? void 0 : _a.call(state, progress, withMetadata3({ peerId }, requestMetadata ? requestMetadata.m : metadata));
    };
    rawAction.onProgress(dispatchReceiveProgress);
    if (kind === "message") {
      let onMessage = (config == null ? void 0 : config.onMessage) ?? null;
      const flushMessages = () => {
        ...
        const handler = onMessage;
        state.pendingMessages.splice(0).forEach(({ payload, peerId, metadata }) => {
          Promise.resolve().then(() => handler(payload, withMetadata3({ peerId }, metadata))).catch((err) => console.error(`${libName3} action handler error:`, err));
        });
      };
      const action2 = {
        send: async (data, options = {}) => {
    ...
    const sendRequestError = (peerId, requestId, error) => {
      responseAction.send(null, peerId, {
        r: requestId,
        e: toErrorMessage3(error, "request failed")
      });
    };
    const respondToIncomingRequest = (request, handler) => {
      removePendingIncomingRequest(request);
      Promise.resolve().then(() => handler(request.payload, {
        ...
        if (response === void 0) throw mkErr3("request handler returned undefined");
        await responseAction.send(response, request.peerId, { r: request.requestId });
      }).catch((err) => sendRequestError(request.peerId, request.requestId, err)).finally(() => request.controller.abort());
    };
    const flushRequests = () => {
    ...
    const requestOne = async (data, options) => {
      ...
      if (!getPeer(target, false)) throw makeActionError3("disconnected", `no active peer with id ${target}`);
      const requestId = genId3(20);
      const handledResponsePromise = new Promise((resolve, reject) => {
        const waiter = {
          peerId: target,
          resolve,
        ...
        const rejectAsAborted = () => {
          clearPendingRequestWaiter(requestId);
          reject(makeActionError3("aborted", "operation aborted"));
        };
        if (signal) {
          waiter.abortHandler = rejectAsAborted;
          signal.addEventListener("abort", rejectAsAborted, { once: true });
      ...
      try {
        ...
        if (waiter && timeoutMs !== void 0) waiter.timer = setTimeout(() => {
          clearPendingRequestWaiter(requestId);
          waiter.reject(makeActionError3("timeout", "request timed out"));
        }, timeoutMs);
        return await handledResponsePromise;
      } catch (err) {
        clearPendingRequestWaiter(requestId);
    ...
    const action = {
      ...
      requestMany: async (data, options) => {
        ...
        return await all3(options.targets.map(async (target) => {
          var _a, _b2;
          try {
            const result = {
              peerId: target,
            ...
            const error = toError3(err, "request failed");
            if (error.kind === "aborted" || !error.kind) throw error;
            const result = error.kind === "timeout" ? {
              peerId: target,
              status: "timeout"
    ...
    rawAction.onMessage((payload, peerId, metadata) => {
      const requestMetadata = getRequestMetadata3(metadata);
      if (!requestMetadata) return;
      queueIncomingRequest(payload, peerId, requestMetadata.m, requestMetadata.r);
    });
    state.action = action;
...
var createMediaIdentityCache3 = () => {
  ...
  const remoteTracksById = /* @__PURE__ */ new Map();
  return {
    getStreamKey: makeKeyGetter3(localStreamKeys),
    getTrackKey: makeKeyGetter3(localTrackKeys),
    rememberRemoteStream: (key, stream, streamId) => {
      remoteStreamsByKey.set(key, stream);
      if (streamId) remoteStreamsById.set(streamId, stream);
    },
...
var createMediaManager3 = ({ iterate, isActive, getSharedMediaPeer }) => {
  const pendingStreamMetas = {};
  const pendingTrackMetas = {};
  const localMedia = createMediaIdentityCache3();
  const listeners = {
    onPeerStream: null,
    onPeerTrack: null
  };
  ...
  return {
    ...
    receiveStreamMeta: (meta, id) => {
      var _a, _b2;
      if (!isActive(id)) return;
      const parsed = toPendingMediaMeta3(meta);
      if (!parsed) return;
      const cached = (_b2 = (_a = getSharedMediaPeer(id)) == null ? void 0 : _a.__trysteroMedia) == null ? void 0 : _b2.getRemoteStream(parsed.key, parsed.streamId);
      if (cached) {
        emitStream(id, parsed.key, cached, parsed.metadata);
...
var unloadEvent3 = "beforeunload";
var defaultHandshakeTimeoutMs3 = 1e4;
var internalNs3 = (ns) => "@_" + ns;
var beforeUnloadRoomCleanups3 = /* @__PURE__ */ new Set();
var cleanupActiveRoomsOnBeforeUnload3 = () => beforeUnloadRoomCleanups3.forEach((cleanup) => cleanup());
var registerBeforeUnloadCleanup3 = (cleanup) => {
  beforeUnloadRoomCleanups3.add(cleanup);
  if (beforeUnloadRoomCleanups3.size === 1) addEventListener(unloadEvent3, cleanupActiveRoomsOnBeforeUnload3);
  return () => {
    beforeUnloadRoomCleanups3.delete(cleanup);
    if (!beforeUnloadRoomCleanups3.size) removeEventListener(unloadEvent3, cleanupActiveRoomsOnBeforeUnload3);
  };
};
var room_default3 = (onPeer, onPeerLeave, onSelfLeave, { onPeerHandshake, onHandshakeError, handshakeTimeoutMs = defaultHandshakeTimeoutMs3, isPassive = false } = {}) => {
  const peerMap = {};
  const activePeerMap = {};
  const pendingPongs = {};
  const listeners = {
  ...
  let unregisterBeforeUnloadCleanup = noOp3;
  let handshakeManager = null;
  const iterate = (targets, f, { includePending = false } = {}) => (targets ? Array.isArray(targets) ? targets : [targets] : keys3(includePending ? peerMap : activePeerMap)).flatMap((id) => {
    const peer = includePending ? peerMap[id] : activePeerMap[id];
    if (!peer) {
      console.warn(`${libName3}: no peer with id ${id} found`);
      return [];
  ...
  const mediaManager = createMediaManager3({
    iterate: (targets, f) => iterate(targets, (id, peer) => f(id, peer)),
    isActive: (id) => Boolean(activePeerMap[id]),
    getSharedMediaPeer: (id) => peerMap[id] ?? null
  });
  const actionManager = createActionManager3({
    getPeer: (id, includePending) => (includePending ? peerMap : activePeerMap)[id],
    getPeerIds: (includePending) => keys3(includePending ? peerMap : activePeerMap),
    canReceiveFromPeer: (id, receiveWhilePending) => Boolean(handshakeManager == null ? void 0 : handshakeManager.canReceiveFromPeer(id, receiveWhilePending))
  });
  const makeActionInternal = actionManager.makeInternalAction;
  const handleData = actionManager.handleData;
  const makeAction = actionManager.makeAction;
  const clearPeerState = (id, reason = mkErr3("peer disconnected")) => {
    var _a;
    const err = toError3(reason, "peer disconnected");
    handshakeManager == null ? void 0 : handshakeManager.clearPeer(id, err);
    delete peerMap[id];
    delete activePeerMap[id];
    actionManager.clearPeer(id, err);
  ...
  const leave = async () => {
    await leaveAction.send("");
    await new Promise((res) => setTimeout(res, 99));
    entries3(peerMap).forEach(([id, peer]) => {
      peer.destroy();
      clearPeerState(id, mkErr3("room left"));
    });
    unregisterBeforeUnloadCleanup();
    onSelfLeave();
  };
  const pingAction = makeActionInternal(internalNs3("ping"));
  const pongAction = makeActionInternal(internalNs3("pong"));
  const signalAction = makeActionInternal(internalNs3("signal"));
  const streamMetaAction = makeActionInternal(internalNs3("stream"));
  const trackMetaAction = makeActionInternal(internalNs3("track"));
  const leaveAction = makeActionInternal(internalNs3("leave"), {
    sendToPending: true,
    receiveWhilePending: true
  });
  const handshakeDataAction = makeActionInternal(internalNs3("hsdata"), {
    sendToPending: true,
    receiveWhilePending: true
  });
  const handshakeReadyAction = makeActionInternal(internalNs3("hsready"), {
    sendToPending: true,
    receiveWhilePending: true
  });
  handshakeManager = createHandshakeManager3({
    ...onPeerHandshake === void 0 ? {} : { onPeerHandshake },
    ...onHandshakeError === void 0 ? {} : { onHandshakeError },
    handshakeTimeoutMs,
    sendHandshakeData: handshakeDataAction.send,
  ...
  streamMetaAction.onMessage((meta, id) => mediaManager.receiveStreamMeta(meta, id));
  trackMetaAction.onMessage((meta, id) => mediaManager.receiveTrackMeta(meta, id));
  leaveAction.onMessage((_, id) => exitPeer(id, void 0, mkErr3("peer left room")));
  handshakeDataAction.onMessage((data, id, metadata) => handshakeManager == null ? void 0 : handshakeManager.receiveHandshakeData(data, id, metadata));
  handshakeReadyAction.onMessage((_, id) => handshakeManager == null ? void 0 : handshakeManager.receiveHandshakeReady(id));
  onPeer((peer, id) => {
    const existingPeer = peerMap[id];
    if (existingPeer) {
      if (existingPeer === peer) return;
      existingPeer.destroy();
      clearPeerState(id, mkErr3("peer replaced"));
    }
    peerMap[id] = peer;
    handshakeManager == null ? void 0 : handshakeManager.addPeer(id, peer);
    peer.setHandlers({
      ...
      close: () => exitPeer(id, peer, mkErr3("peer disconnected")),
      error: (err) => {
        console.error(`${libName3} peer error:`, err);
        exitPeer(id, peer, err);
      }
    });
    handshakeManager == null ? void 0 : handshakeManager.start(id, peer);
  });
  if (isBrowser3) unregisterBeforeUnloadCleanup = registerBeforeUnloadCleanup3(() => leave().catch(noOp3));
  return {
    makeAction,
    leave,
    ping: async (id) => {
      if (!activePeerMap[id]) throw mkErr3(`no active peer with id ${id}`);
      const start = Date.now();
      await new Promise((resolve, reject) => {
        const queue = pendingPongs[id] ?? (pendingPongs[id] = []);
        const clearFromQueue = () => {
        ...
        queue.push(waiter);
        pingAction.send("", id).catch((err) => waiter.reject(toError3(err, "peer disconnected")));
      });
      return Date.now() - start;
    },
    isPassive: () => isPassive,
    getPeers: () => fromEntries3(entries3(activePeerMap).map(([id, peer]) => [id, peer.connection])),
    addStream: (stream, options = {}) => mediaManager.addStream(stream, options, streamMetaAction.send),
    removeStream: (stream, options = {}) => {
      mediaManager.removeStream(stream, options.target);
    },
    ...
    set onPeerJoin(handler) {
      listeners.onPeerJoin = handler;
      if (handler) keys3(activePeerMap).forEach((peerId) => handler(peerId));
    },
    get onPeerLeave() {
      return listeners.onPeerLeave;
    },
...
var roomFrameVersion3 = 1;
var roomPresenceFrameVersion3 = 2;
var wrapRoomFrame3 = (roomToken, data) => {
  const tokenBytes = encodeBytes3(roomToken);
  const frame = new Uint8Array(3 + tokenBytes.byteLength + data.byteLength);
  frame[0] = roomFrameVersion3;
  frame[1] = tokenBytes.byteLength >>> 8 & 255;
  frame[2] = tokenBytes.byteLength & 255;
  frame.set(tokenBytes, 3);
  frame.set(data, 3 + tokenBytes.byteLength);
...
var wrapRoomPresenceFrame3 = (roomToken, isPresent) => {
  const tokenBytes = encodeBytes3(roomToken);
  const frame = new Uint8Array(4 + tokenBytes.byteLength);
  frame[0] = roomPresenceFrameVersion3;
  frame[1] = Number(isPresent);
  frame[2] = tokenBytes.byteLength >>> 8 & 255;
  frame[3] = tokenBytes.byteLength & 255;
  frame.set(tokenBytes, 4);
...
var unwrapFrame3 = (data) => {
  const buffer = new Uint8Array(data);
  if (buffer.byteLength < 3) return null;
  if (buffer[0] === roomFrameVersion3) {
    const tokenSize2 = (buffer[1] ?? 0) << 8 | (buffer[2] ?? 0);
    const headerSize2 = 3 + tokenSize2;
    if (tokenSize2 <= 0 || buffer.byteLength < headerSize2) return null;
    return {
      type: "room",
      roomToken: decodeBytes3(buffer.subarray(3, headerSize2)),
      payload: buffer.subarray(headerSize2).slice().buffer
    };
  }
  if (buffer[0] !== roomPresenceFrameVersion3 || buffer.byteLength < 4) return null;
  const tokenSize = (buffer[2] ?? 0) << 8 | (buffer[3] ?? 0);
  const headerSize = 4 + tokenSize;
  if (tokenSize <= 0 || buffer.byteLength < headerSize) return null;
  return {
    type: "presence",
    roomToken: decodeBytes3(buffer.subarray(4, headerSize)),
    isPresent: buffer[1] === 1
  };
};
var isPeerUnderlyingStale3 = (peer) => {
  const { connection, channel } = peer;
  return peer.isDead || connection.connectionState === "closed" || connection.connectionState === "failed" || connection.iceConnectionState === "closed" || connection.iceConnectionState === "failed" || (channel == null ? void 0 : channel.readyState) === "closing" || (channel == null ? void 0 : channel.readyState) === "closed";
};
var getConnectedPeerHealth3 = (peer) => {
  if (isPeerUnderlyingStale3(peer)) return "stale";
  const { channel } = peer;
  if (!channel || channel.readyState !== "open") return "transient";
  return "live";
};
var SharedPeerManager3 = class {
  constructor() {
    __publicField(this, "byApp", {});
    __publicField(this, "roomPresenceHandlers", {});
  }
  ...
  isPeerStale(peer) {
    return isPeerUnderlyingStale3(peer);
  }
  getHealth(peer) {
    return this.isPeerStale(peer) ? "stale" : "live";
  }
  ...
  sendRoomPresence(shared, roomToken, isPresent) {
    if (shared.isClosing || shared.peer.isDead) return;
    shared.peer.sendData(wrapRoomPresenceFrame3(roomToken, isPresent));
  }
  clear(appId, peerId, { destroyPeer }) {
    const map = this.byApp[appId];
    const shared = map == null ? void 0 : map[peerId];
    if (!shared || shared.isClosing) return;
    shared.idleTimer = resetTimer3(shared.idleTimer);
    shared.isClosing = true;
    if (destroyPeer && !shared.peer.isDead) shared.peer.destroy();
    const bindings = values3(shared.bindings);
    shared.bindings = {};
    shared.bindingsByToken = {};
    shared.controlRoomId = null;
    delete map[peerId];
    ...
    shared.pendingDataByToken.clear();
    shared.remoteRoomTokens.clear();
    if (keys3(map).length === 0) delete this.byApp[appId];
  }
  register(appId, peerId, peer, idleMs) {
    const map = this.getMap(appId);
    const existing = map[peerId];
    if (existing) {
      existing.idleTimer = resetTimer3(existing.idleTimer);
      if (existing.peer === peer) return existing;
      this.clear(appId, peerId, { destroyPeer: true });
    }
    const shared = {
      ...
      streamOwners: /* @__PURE__ */ new Map(),
      trackOwners: /* @__PURE__ */ new Map(),
      media: createMediaIdentityCache3(),
      idleMs,
      isClosing: false
    };
    peer.setHandlers({
  ...
  bind(roomId, roomTokenPromise, shared, { onDetach }) {
    ...
    const detachBinding = () => {
      ...
      delete shared.bindings[roomId];
      if (binding.roomToken && shared.bindingsByToken[binding.roomToken] === binding) delete shared.bindingsByToken[binding.roomToken];
      if (shared.controlRoomId === roomId) shared.controlRoomId = keys3(shared.bindings)[0] ?? null;
      onDetach();
      this.scheduleIdleTimer(shared);
    };
    const proxy = {
      ...
      sendData: (data) => {
        ...
        shared.peer.sendData(wrapRoomFrame3(binding.roomToken, data));
      },
      destroy: () => detachBinding(),
      setHandlers: (newHandlers) => {
        const { signal, ...rest } = newHandlers;
    ...
    roomTokenPromise.then((roomToken) => {
      ...
      binding.pendingSendData.splice(0).forEach((payload) => shared.peer.sendData(wrapRoomFrame3(roomToken, payload)));
      this.flushBindingQueues(binding);
    });
    return {
      proxy,
  ...
  scheduleIdleTimer(shared) {
    if (shared.isClosing || keys3(shared.bindings).length > 0) return;
    shared.idleTimer = resetTimer3(shared.idleTimer);
    shared.idleTimer = setTimeout(() => {
      var _a;
      const current = (_a = this.byApp[shared.appId]) == null ? void 0 : _a[shared.peerId];
      if (!current || keys3(current.bindings).length > 0) return;
      this.clear(shared.appId, shared.peerId, { destroyPeer: true });
    }, shared.idleMs);
  }
  getSignalBinding(shared) {
    ...
    const fallback = values3(shared.bindings).find((binding) => Boolean(binding.handlers.signal));
    if (!fallback) return null;
    shared.controlRoomId = fallback.roomId;
    return fallback;
  }
  ...
  dispatchData(shared, data) {
    var _a, _b2;
    const decoded = unwrapFrame3(data);
    if (!decoded) return;
    if (decoded.type === "presence") {
      if (decoded.isPresent) shared.remoteRoomTokens.add(decoded.roomToken);
      else shared.remoteRoomTokens.delete(decoded.roomToken);
  ...
  dispatchTrack(shared, track, stream) {
    values3(shared.bindings).forEach((binding) => {
      var _a, _b2, _c2, _d;
      if (binding.handlers.track || binding.handlers.stream) {
        (_b2 = (_a = binding.handlers).track) == null ? void 0 : _b2.call(_a, track, stream);
        (_d = (_c2 = binding.handlers).stream) == null ? void 0 : _d.call(_c2, stream);
...
var offerPostAnswerTtlMs3 = 23333;
var offerIdSize3 = 12;
var disconnectedPeerGraceMs3 = 7533;
var answeringTtlMs3 = 23333;
var legacyCandidateKey3 = "__legacy__";
var offerRelayPlaceholder3 = "offer-placeholder";
var signalKeys4 = [
  "offer",
  "answer",
  "candidate"
...
var getString4 = (payload, key) => typeof payload[key] === "string" && payload[key] ? payload[key] : void 0;
var hasInvalidSignalField4 = (payload) => signalKeys4.some((key) => key in payload && (typeof payload[key] !== "string" || payload[key] === ""));
var publishCipheredSignalingMessage3 = (ctx, signal, peerTopic, signalPeer, buildPayload, stillValid) => {
  ctx.toCipher(signal).then((encryptedSignal) => {
    if (ctx.isLeaving() || !stillValid()) return;
    signalPeer(peerTopic, toJson3(buildPayload(encryptedSignal.sdp)));
  });
};
var makeState3 = () => ({
  status: "idle",
...
var hasTurnServer3 = (config) => {
  var _a;
  return [...config.turnConfig ?? [], ...((_a = config.rtcConfig) == null ? void 0 : _a.iceServers) ?? []].some(({ urls }) => {
    return (Array.isArray(urls) ? urls : [urls]).some((url) => /^turns?:/i.test(url));
  });
};
var getSdpExchangeConnectionError3 = (peerId, config) => `could not connect to peer ${peerId} after exchanging SDP; ${hasTurnServer3(config) ? "check that your TURN server URLs and credentials are reachable by both peers" : "configure TURN servers with turnConfig or rtcConfig.iceServers"}`;
var reportSdpExchangeConnectionFailure3 = (ctx, state, peerId) => {
  var _a;
  if (ctx.isLeaving() || state.connectedPeer || state.connectionErrorReported) return;
  state.connectionErrorReported = true;
  (_a = ctx.onJoinError) == null ? void 0 : _a.call(ctx, {
    error: getSdpExchangeConnectionError3(peerId, ctx.config),
    appId: ctx.appId,
    peerId,
    roomId: ctx.roomId
  });
};
var getState3 = (peerStates, peerId) => peerStates[peerId] ?? (peerStates[peerId] = makeState3());
var updateStatus3 = (state) => {
  if (state.connectedPeer) state.status = "connected";
  else if (state.answeringPeer) state.status = "answering";
  else if (state.offerPeer || state.offerRelays.some(Boolean)) state.status = "offering";
  else state.status = "idle";
};
var clearAnswering3 = (state, peer) => {
  if (state.answeringPeer === peer) {
    state.answeringExpiryTimer = resetTimer3(state.answeringExpiryTimer);
    state.answeringPeer = null;
    state.answerSent = false;
    updateStatus3(state);
  }
};
var clearConnectedPeer3 = (state, peerId, _reason) => {
  if (!state.connectedPeer) return;
  if (!state.connectedPeer.isDead) state.connectedPeer.destroy();
  state.connectedPeer = null;
  state.connectedPeerUnhealthySinceMs = null;
...
var clearOfferRelay3 = (state, relayId) => {
  state.offerRelayTimers[relayId] = resetTimer3(state.offerRelayTimers[relayId]);
  if (state.offerRelays[relayId]) {
    state.offerRelays[relayId] = void 0;
    updateStatus3(state);
  }
};
var clearOfferRelayIfPlaceholder3 = (state, relayId) => {
  if ((state == null ? void 0 : state.offerRelays[relayId]) === offerRelayPlaceholder3) clearOfferRelay3(state, relayId);
};
var hasRemoteDescription3 = (peer) => {
  if (peer.isDead || peer.connection.connectionState === "closed") return true;
  try {
    return Boolean(peer.connection.remoteDescription);
  } catch {
...
var resetOfferState3 = (state, offerPool) => {
  const previousOfferAnswered = state.offerAnswered;
  state.offerExpiryTimer = resetTimer3(state.offerExpiryTimer);
  state.offerInitPromise = null;
  state.offerRelays.forEach((_, relayId) => clearOfferRelay3(state, relayId));
  state.offerRelays = [];
  state.offerSignalRelays = [];
  state.offerRelayTimers = [];
  state.offerSignalBacklog = [];
  if (state.offerPeer && state.offerPeer !== state.connectedPeer) if (previousOfferAnswered || hasRemoteDescription3(state.offerPeer)) {
    if (!state.offerPeer.isDead) state.offerPeer.destroy();
  } else offerPool.recycle(state.offerPeer);
  state.offerPeer = null;
  state.offerId = null;
...
var scheduleAnsweringExpiry3 = (ctx, state, peerId, peer) => {
  resetTimer3(state.answeringExpiryTimer);
  state.answeringExpiryTimer = setTimeout(() => {
    const current = ctx.peerStates[peerId];
    if (!current || current.connectedPeer || current.answeringPeer !== peer) return;
    if (current.answerSent) reportSdpExchangeConnectionFailure3(ctx, current, peerId);
    peer.destroy();
    clearAnswering3(current, peer);
    ctx.checkDeactivate();
  }, answeringTtlMs3);
};
var flushBufferedCandidates3 = async (state, peer, offerId) => {
  const bufferKeys = offerId ? [offerId, legacyCandidateKey3] : [legacyCandidateKey3];
  for (const key of bufferKeys) {
    const buffered = state.pendingCandidates[key];
    if (!(buffered == null ? void 0 : buffered.length)) continue;
    delete state.pendingCandidates[key];
...
var scheduleOfferExpiry3 = (ctx, state, peerId, ttlMs = offerTtl3) => {
  resetTimer3(state.offerExpiryTimer);
  const offerId = state.offerId;
  state.offerExpiryTimer = setTimeout(() => {
    const current = ctx.peerStates[peerId];
    if (!current || current.connectedPeer || current.offerId !== offerId) return;
    if (current.offerAnswered) reportSdpExchangeConnectionFailure3(ctx, current, peerId);
    resetOfferState3(current, ctx.offerPool);
    ctx.checkDeactivate();
  }, ttlMs);
};
var ensureOffer3 = (ctx, state, peerId, relayId) => {
  if (state.offerPeer && state.offerId && state.offerSdp) return Promise.resolve({
    peer: state.offerPeer,
    offer: state.offerSdp,
    offerId: state.offerId
  ...
  state.offerInitPromise = (async () => {
    const firstOffer = (await ctx.offerPool.checkout(1, false, ctx.encryptOffer))[0];
    if (!firstOffer) throw mkErr3("failed to allocate offer peer");
    const { peer, offer } = firstOffer;
    state.offerPeer = peer;
    state.offerId = genId3(offerIdSize3);
    state.offerSdp = offer;
    state.offerAnswered = false;
    state.connectionErrorReported = false;
    state.offerSignalBacklog = [];
    ...
    const onOfferPeerClosedOrError = () => {
      if (state.offerPeer === peer && !state.connectedPeer) {
        if (state.offerAnswered) reportSdpExchangeConnectionFailure3(ctx, state, peerId);
        resetOfferState3(state, ctx.offerPool);
      }
      ctx.disconnectPeer(peer, peerId);
      ctx.checkDeactivate();
    };
    ...
    scheduleOfferExpiry3(ctx, state, peerId);
    return {
      peer,
      offer,
      offerId: state.offerId
...
var handleAnnouncement3 = async (ctx, relayId, peerId, shared, signalPeer) => {
  if (shared) {
    ctx.attachSharedPeerToRoom(peerId, shared);
    return;
  }
  const state = ctx.peerStates[peerId];
  if (!state || state.connectedPeer || state.answeringPeer || state.offerAnswered) {
    clearOfferRelayIfPlaceholder3(state, relayId);
    return;
  }
  if (state.offerRelays[relayId] !== offerRelayPlaceholder3) return;
  const [peerTopic, offerInfo] = await all3([sha13(topicPath3(ctx.rootTopicPlaintext, peerId)), ensureOffer3(ctx, state, peerId, relayId)]);
  if (ctx.isLeaving()) return;
  if (state.connectedPeer || state.answeringPeer || state.offerAnswered || state.offerRelays[relayId] !== offerRelayPlaceholder3) {
    clearOfferRelayIfPlaceholder3(state, relayId);
    return;
  }
  state.offerRelayTimers[relayId] = resetTimer3(state.offerRelayTimers[relayId]);
  state.offerRelays[relayId] = true;
  updateStatus3(state);
  state.offerRelayTimers[relayId] = setTimeout(() => prunePendingOffer3(ctx, peerId, relayId), (ctx.announceIntervals[relayId] ?? ctx.announceIntervalMs) * 0.9);
  let didSendOffer = false;
  state.offerSignalRelays[relayId] = (signal) => {
    if (!didSendOffer) return;
    if (ctx.isLeaving() || state.connectedPeer || state.offerPeer !== offerInfo.peer || state.offerId !== offerInfo.offerId || signal.type !== "candidate") return;
    publishCipheredSignalingMessage3(ctx, signal, peerTopic, signalPeer, (sdp) => ({
      peerId: selfId3,
      offerId: offerInfo.offerId,
      candidate: sdp,
      ...ctx.isPassive ? { passive: true } : {}
...
var handleOffer3 = async (ctx, relayId, peerId, offer, offerId, hasOutgoingOfferHint, signalPeer) => {
  var _a;
  const state = getState3(ctx.peerStates, peerId);
  if (state.answeringPeer || state.offerAnswered) return;
  const hasTrackedOutgoingOffer = Boolean(state.offerPeer || state.offerRelays.some(Boolean));
  if ((hasTrackedOutgoingOffer || hasOutgoingOfferHint) && selfId3 < peerId) return;
  if (hasTrackedOutgoingOffer) resetOfferState3(state, ctx.offerPool);
  const answerPeer = ctx.initPeer(false, ctx.config);
  state.answeringPeer = answerPeer;
  state.answerSent = false;
  state.connectionErrorReported = false;
  scheduleAnsweringExpiry3(ctx, state, peerId, answerPeer);
  updateStatus3(state);
  const onAnswerPeerClosedOrError = () => {
    if (state.answeringPeer === answerPeer && !state.connectedPeer && state.answerSent) reportSdpExchangeConnectionFailure3(ctx, state, peerId);
    clearAnswering3(state, answerPeer);
    ctx.disconnectPeer(answerPeer, peerId);
    ctx.checkDeactivate();
  };
  answerPeer.setHandlers({
  ...
  const peerTopic = await sha13(topicPath3(ctx.rootTopicPlaintext, peerId));
  if (ctx.isLeaving()) return;
  answerPeer.setHandlers({ signal: (signal) => {
    if (ctx.isLeaving() || state.answeringPeer !== answerPeer || answerPeer.isDead) return;
    if (signal.type !== "answer" && signal.type !== "candidate") return;
    publishCipheredSignalingMessage3(ctx, signal, peerTopic, signalPeer, (sdp) => {
      const payloadToSend = { peerId: selfId3 };
      if (signal.type === "answer") {
        state.answerSent = true;
        payloadToSend["answer"] = sdp;
      } else payloadToSend["candidate"] = sdp;
  ...
  await answerPeer.signal(plainOffer);
  await flushBufferedCandidates3(state, answerPeer, offerId);
};
var handleCandidate3 = async (ctx, peerId, candidate, offerId, peer) => {
  var _a;
  let plainCandidate;
  try {
    plainCandidate = await ctx.toPlain({
  ...
  const targetPeer = peer && !peer.isDead ? peer : offerPeerMatch ?? answeringPeer ?? fallbackOfferPeer;
  if (!targetPeer || targetPeer.isDead) {
    const pendingKey = offerId ?? legacyCandidateKey3;
    ((_a = state.pendingCandidates)[pendingKey] ?? (_a[pendingKey] = [])).push(plainCandidate);
    return;
  }
  targetPeer.signal(plainCandidate);
};
var handleAnswer3 = async (ctx, relayId, peerId, answer, offerId, peer) => {
  var _a;
  let plainAnswer;
  try {
    plainAnswer = await ctx.toPlain({
  ...
  if (peer) {
    ...
    if (!state || !state.offerPeer || state.offerAnswered || offerId && state.offerId && offerId !== state.offerId || state.offerPeer.isDead) return;
    state.offerAnswered = true;
    scheduleOfferExpiry3(ctx, state, peerId, offerPostAnswerTtlMs3);
    state.offerPeer.signal(plainAnswer);
  }
};
var prunePendingOffer3 = (ctx, peerId, relayId) => {
  const state = ctx.peerStates[peerId];
  if (!state || state.connectedPeer) return;
  if (state.offerRelays[relayId]) {
    clearOfferRelay3(state, relayId);
    ctx.checkDeactivate();
  }
};
var createSignalHandler3 = (ctx) => (relayId) => async (topic, msg, signalPeer) => {
  var _a;
  if (ctx.isLeaving()) return;
  const payload = toPayload4(msg);
  if (!payload || hasInvalidSignalField4(payload)) return;
  const peerId = getString4(payload, "peerId") ?? "";
  const offer = getString4(payload, "offer");
  const answer = getString4(payload, "answer");
  const candidate = getString4(payload, "candidate");
  const offerId = getString4(payload, "offerId");
  const peer = payload["peer"];
  const hasOutgoingOfferHint = payload["hasOutgoingOffer"] === true;
  const remoteIsPassive = payload["passive"] === true;
  if (!peerId || peerId === selfId3) return;
  const [rootTopic, selfTopic] = await all3([ctx.rootTopicP, ctx.selfTopicP]);
  if (ctx.isLeaving()) return;
  if (topic !== rootTopic && topic !== selfTopic) return;
  if (ctx.isPassive && remoteIsPassive) return;
  if (ctx.isPassive && !ctx.isActive && !answer && !candidate) {
  ...
  const connectedPeer = state == null ? void 0 : state.connectedPeer;
  if (connectedPeer && state) {
    const health = getConnectedPeerHealth3(connectedPeer);
    if (health === "live") {
      state.connectedPeerUnhealthySinceMs = null;
      return;
    }
    if (health === "stale") clearConnectedPeer3(state, peerId, "message-from-stale-peer");
    else {
      const nowMs = Date.now();
      const unhealthySinceMs = state.connectedPeerUnhealthySinceMs ?? nowMs;
      state.connectedPeerUnhealthySinceMs = unhealthySinceMs;
      if (nowMs - unhealthySinceMs < disconnectedPeerGraceMs3) return;
      clearConnectedPeer3(state, peerId, "message-from-prolonged-disconnect");
    }
  }
  let shared = ctx.sharedPeers.get(ctx.appId, peerId);
  if (shared && ctx.sharedPeers.getHealth(shared.peer) === "stale") {
  ...
  const isAnnouncement = Boolean(peerId && !offer && !answer && !candidate);
  if (isAnnouncement && !shared) {
    const announcePeerState = getState3(ctx.peerStates, peerId);
    const shouldLeadOffer = selfId3 < peerId;
    if (announcePeerState.answeringPeer || announcePeerState.connectedPeer || announcePeerState.offerAnswered) return;
    if (!shouldLeadOffer && !announcePeerState.offerPeer) {
      const peerSelfTopic = await sha13(topicPath3(ctx.rootTopicPlaintext, peerId));
      if (!ctx.isLeaving() && !announcePeerState.connectedPeer) signalPeer(peerSelfTopic, toJson3({ peerId: selfId3 }));
      return;
    }
    if (announcePeerState.offerRelays[relayId]) return;
    announcePeerState.offerRelays[relayId] = offerRelayPlaceholder3;
    updateStatus3(announcePeerState);
  }
  if (shared && (offer || answer || candidate)) {
    if (shared.bindings[ctx.roomId]) return;
    ctx.attachSharedPeerToRoom(peerId, shared);
  ...
  if (isAnnouncement) return handleAnnouncement3(ctx, relayId, peerId, shared, signalPeer);
  if (offer) return handleOffer3(ctx, relayId, peerId, offer, offerId, hasOutgoingOfferHint, signalPeer);
  if (candidate) return handleCandidate3(ctx, peerId, candidate, offerId, peer);
  if (answer) return handleAnswer3(ctx, relayId, peerId, answer, offerId, peer);
};

// ../../../../node_modules/@trystero-p2p/mqtt/node_modules/@trystero-p2p/core/dist/strategy.mjs
var announceIntervalMs3 = 5333;
var announceWarmupIntervalsMs3 = [
  233,
  533,
  1333
];
var passiveActivationGraceMs3 = 7533;
var sharedPeerIdleMsDefault3 = 123333;
var strategy_default3 = ({ init, subscribe, announce, deactivate }) => {
  const occupiedRooms = {};
  const roomRegistrations = {};
  const roomIdsByToken = {};
  const roomPresenceHandlerCleanups = {};
  const sharedPeers = new SharedPeerManager3();
  const hasActiveRooms = () => values3(occupiedRooms).some((rooms) => keys3(rooms).length > 0);
  const getRoomRegistrations = (appId) => roomRegistrations[appId] ?? (roomRegistrations[appId] = {});
  const getRoomIdsByToken = (appId) => roomIdsByToken[appId] ?? (roomIdsByToken[appId] = {});
  const advertiseRoomPresence = (shared, roomToken, isPresent) => {
    if (sharedPeers.getHealth(shared.peer) === "live") sharedPeers.sendRoomPresence(shared, roomToken, isPresent);
  ...
  const advertiseKnownRoomsToShared = (appId, shared) => {
    entries3(roomRegistrations[appId] ?? {}).forEach(([roomId, registration]) => {
      if (!registration.shouldAdvertise()) return;
      const { roomToken, roomTokenPromise } = registration;
      if (roomToken) {
        advertiseRoomPresence(shared, roomToken, true);
  ...
  const advertiseRoomPresenceToAll = (appId, roomToken, isPresent) => values3(sharedPeers.getMap(appId)).forEach((shared) => advertiseRoomPresence(shared, roomToken, isPresent));
  const ensureRoomPresenceHandler = (appId) => {
    if (roomPresenceHandlerCleanups[appId]) return;
    roomPresenceHandlerCleanups[appId] = sharedPeers.setRoomPresenceHandler(appId, (peerId, roomToken, isPresent) => {
      var _a, _b2, _c2;
  ...
  const cleanupRoomPresenceHandler = (appId) => {
    var _a;
    if (occupiedRooms[appId] && keys3(occupiedRooms[appId]).length > 0) return;
    (_a = roomPresenceHandlerCleanups[appId]) == null ? void 0 : _a.call(roomPresenceHandlerCleanups);
    delete roomPresenceHandlerCleanups[appId];
    delete roomRegistrations[appId];
    delete roomIdsByToken[appId];
  ...
  let initPromises = [];
  let offerPool = null;
  let cleanupWatchOnline = noOp3;
  return (config, roomId, callbacks) => {
    var _a, _b2;
    if (!config) throw mkErr3("requires a config map as the first argument");
    if (callbacks && typeof callbacks !== "object") throw mkErr3("third argument must be a callbacks object");
    const { appId } = config;
    const onJoinError = callbacks == null ? void 0 : callbacks.onJoinError;
    const onPeerHandshake = callbacks == null ? void 0 : callbacks.onPeerHandshake;
    const handshakeTimeoutMs = callbacks == null ? void 0 : callbacks.handshakeTimeoutMs;
    if (!appId) throw mkErr3("config map is missing appId field");
    if (!roomId) throw mkErr3("roomId argument required");
    if (handshakeTimeoutMs !== void 0 && (!Number.isFinite(handshakeTimeoutMs) || handshakeTimeoutMs <= 0)) throw mkErr3("handshakeTimeoutMs must be a positive number");
    if ((_a = occupiedRooms[appId]) == null ? void 0 : _a[roomId]) return occupiedRooms[appId][roomId];
    ensureRoomPresenceHandler(appId);
    const rootTopicPlaintext = topicPath3(libName3, appId, roomId);
    const rootTopicP = sha13(rootTopicPlaintext);
    const selfTopicP = sha13(topicPath3(rootTopicPlaintext, selfId3));
    const key = genKey3(config.password ?? "", appId, roomId);
    const roomNamespacePromise = deriveRoomNamespace3(appId, roomId);
    const sharedPeerIdleMs = config._test_only_sharedPeerIdleMs ?? sharedPeerIdleMsDefault3;
    let didLeaveRoom = false;
    const withKey = (f) => async (signal) => ({
      type: signal.type,
      sdp: await f(key, signal.sdp)
    });
    const toPlain = withKey(decrypt3);
    const toCipher = withKey(encrypt3);
    const sharedPeerMap = sharedPeers.getMap(appId);
    const makeOffer = () => peer_default3(true, config);
    let reannounceOnDisconnect = false;
    offerPool || (offerPool = new OfferPool3(makeOffer));
    const pool = offerPool;
    const encryptOffer = async (peer) => {
      const plainOffer = await peer.getOffer(Date.now() - peer.created > offerTtl3);
      if (!plainOffer || plainOffer.type !== "offer") throw mkErr3("failed to get offer for peer");
      return (await toCipher(plainOffer)).sdp;
    };
    const attachSharedPeerToRoom = (peerId, shared) => {
      const state = getState3(ctx.peerStates, peerId);
    ...
    const disconnectPeer = (peer, peerId) => {
      ...
      const state = ctx.peerStates[peerId];
      if ((state == null ? void 0 : state.connectedPeer) === peer) {
        clearConnectedPeer3(state, peerId, "close-event");
        checkDeactivate();
        if (!isPassive && reannounceOnDisconnect) (_a2 = ctx.requeueAnnounce) == null ? void 0 : _a2.call(ctx);
      }
    };
    ...
    let roomRegistration = null;
    let passiveActivationTimeout;
    let deactivateRelayAnnouncements = noOp3;
    const checkDeactivate = () => {
      if (!isPassive || !ctx.isActive) return;
      let hasActiveWork = false;
      entries3(ctx.peerStates).forEach(([peerId, state]) => {
        if (state.connectedPeer || state.answeringPeer || state.offerInitPromise || state.offerPeer || state.offerRelays.some(Boolean)) hasActiveWork = true;
        else if (state.status === "idle") delete ctx.peerStates[peerId];
      });
      if (!hasActiveWork) {
        ctx.isActive = false;
        passiveActivationTimeout = resetTimer3(passiveActivationTimeout);
        announceTimeouts.forEach(resetTimer3);
        announceTimeouts.length = 0;
        deactivateRelayAnnouncements();
        if (roomRegistration == null ? void 0 : roomRegistration.roomToken) advertiseRoomPresenceToAll(appId, roomRegistration.roomToken, false);
      }
    ...
    const ctx = {
      ...
      offerPool: pool,
      encryptOffer,
      initPeer: peer_default3,
      connectPeer,
      disconnectPeer,
      attachSharedPeerToRoom,
      checkDeactivate,
      announceIntervals: [],
      announceIntervalMs: announceIntervalMs3
    };
    const strategyContext = {
      config,
      appId,
    ...
    const handleMessage = createSignalHandler3(ctx);
    if (!didInit) {
      const initRes = init(config);
      initPromises = (Array.isArray(initRes) ? initRes : [initRes]).map((value) => Promise.resolve(value));
      didInit = true;
      cleanupWatchOnline = ((_b2 = config.relayConfig) == null ? void 0 : _b2.manualReconnection) ? noOp3 : watchOnline3();
    }
    if (!isPassive && !pool.isActive) pool.warmup();
    ctx.announceIntervals = initPromises.map(() => announceIntervalMs3);
    const announceScheduleIntervals = initPromises.map(() => announceIntervalMs3);
    const announceAttemptCounts = initPromises.map(() => 0);
    const announceErrorStreaks = initPromises.map(() => 0);
    const announceTimeouts = [];
    const unsubFns = initPromises.map(async (relayP, i) => subscribe(await relayP, await rootTopicP, await selfTopicP, handleMessage(i), (n) => pool.getOffers(n, encryptOffer), strategyContext));
    all3([rootTopicP, selfTopicP]).then(([rootTopic, selfTopic]) => {
      if (didLeaveRoom) return;
      const queueAnnounce = async (relay, i) => {
        var _a2;
        if (didLeaveRoom) return;
        ...
        try {
          ...
          const errorStreak = announceErrorStreaks[i] ?? 0;
          if (errorStreak === 0 && ((_a2 = config.relayConfig) == null ? void 0 : _a2.warnOnRelayFailure) !== false) console.warn(`${libName3}: announce failed - ${toErrorMessage3(error, "")}`);
          announceErrorStreaks[i] = errorStreak + 1;
        }
        if (didLeaveRoom || isPassive && !ctx.isActive) return;
        if (announceResult && typeof announceResult !== "number" && "stopAnnouncing" in announceResult) return;
        ...
        const announceAttempt = announceAttemptCounts[i] ?? 0;
        announceAttemptCounts[i] = announceAttempt + 1;
        const currentInterval = announceScheduleIntervals[i] ?? announceIntervalMs3;
        const warmupDelay = announceWarmupIntervalsMs3[announceAttempt];
        announceTimeouts[i] = setTimeout(() => {
          queueAnnounce(relay, i);
        }, typeof warmupDelay === "number" ? Math.min(currentInterval, warmupDelay) : currentInterval);
      };
      ...
      ctx.requeueAnnounce = () => {
        ...
        if (!pool.isActive) pool.warmup();
        if (roomRegistration == null ? void 0 : roomRegistration.roomToken) advertiseRoomPresenceToAll(appId, roomRegistration.roomToken, true);
        passiveActivationTimeout = setTimeout(checkDeactivate, passiveActivationGraceMs3);
        initPromises.forEach(async (relayP, i) => {
          const relay = await relayP;
          if (relay && !didLeaveRoom) {
            announceAttemptCounts[i] = 0;
    ...
    let onPeerConnect = noOp3;
    const { compose } = createPasswordHandshake3(config.password ?? "", appId, roomId);
    const composedPeerHandshake = compose(onPeerHandshake);
    const roomOptions = {
      ...composedPeerHandshake ? { onPeerHandshake: composedPeerHandshake } : {},
      ...handshakeTimeoutMs === void 0 ? {} : { handshakeTimeoutMs },
    ...
    occupiedRooms[appId] ?? (occupiedRooms[appId] = {});
    const appRoomRegistrations = getRoomRegistrations(appId);
    const joinedRoom = room_default3((f) => onPeerConnect = f, (id) => {
      if (didLeaveRoom) return;
      const state = ctx.peerStates[id];
      if (state == null ? void 0 : state.connectedPeer) {
        state.connectedPeer = null;
      ...
      if (registration == null ? void 0 : registration.roomToken) {
        advertiseRoomPresenceToAll(appId, registration.roomToken, false);
        (_b3 = roomIdsByToken[appId]) == null ? true : delete _b3[registration.roomToken];
        if (roomIdsByToken[appId] && !keys3(roomIdsByToken[appId]).length) delete roomIdsByToken[appId];
      }
      if (roomRegistrations[appId]) {
        delete roomRegistrations[appId][roomId];
        if (!keys3(roomRegistrations[appId]).length) delete roomRegistrations[appId];
      }
      entries3(ctx.peerStates).forEach(([peerId, state]) => {
        state.answeringExpiryTimer = resetTimer3(state.answeringExpiryTimer);
        if (state.connectedPeer && !state.connectedPeer.isDead) {
      ...
      if (occupiedRooms[appId]) {
        delete occupiedRooms[appId][roomId];
        if (keys3(occupiedRooms[appId]).length === 0) delete occupiedRooms[appId];
      }
      announceTimeouts.forEach(resetTimer3);
      passiveActivationTimeout = resetTimer3(passiveActivationTimeout);
      unsubFns.forEach(async (f) => {
    ...
    roomNamespacePromise.then((roomToken) => {
      ...
      registration.roomToken = roomToken;
      getRoomIdsByToken(appId)[roomToken] = roomId;
      values3(sharedPeerMap).forEach((shared) => {
        if (shared.remoteRoomTokens.has(roomToken)) attachSharedPeerToRoom(shared.peerId, shared);
      });
      if (!isPassive || ctx.isActive) advertiseRoomPresenceToAll(appId, roomToken, true);
    });
...
var defaultSteadyAnnounceIntervalMs2 = 6e4;
var toPayload5 = (msg) => {
  if (typeof msg === "string") try {
    const parsed = fromJson3(msg);
    return parsed && typeof parsed === "object" ? parsed : null;
...
var getString5 = (payload, key) => typeof payload[key] === "string" && payload[key] ? payload[key] : void 0;
var hasInvalidSignalField5 = (payload) => signalKeys5.some((key) => key in payload && (typeof payload[key] !== "string" || payload[key] === ""));
var shouldActivatePassiveRoom2 = (msg) => {
  const payload = toPayload5(msg);
  if (!payload || hasInvalidSignalField5(payload)) return false;
  const peerId = getString5(payload, "peerId");
  return Boolean(peerId && peerId !== selfId3 && payload["passive"] !== true && !getString5(payload, "answer") && !getString5(payload, "candidate"));
};
var requireContext2 = (context) => {
  if (!context) throw mkErr3("topic strategy missing room context");
  return context;
};
var subscriptionContext2 = (context, kind, rootTopic, selfTopic) => ({
  kind,
  appId: context.appId,
  roomId: context.roomId,
  rootTopic,
...
var publishContext2 = (context, kind, rootTopic, selfTopic) => ({
  kind,
  appId: context.appId,
  roomId: context.roomId,
  rootTopic,
...
var topic_strategy_default3 = ({ steadyAnnounceIntervalMs: steadyAnnounceIntervalMs2 = defaultSteadyAnnounceIntervalMs2, reannounceOnDisconnect = true, init, subscribeTopic, publishTopic, unpublishTopic }) => strategy_default3({
  init,
  subscribe: async (relay, rootTopic, selfTopic, onMessage, _getOffers, rawContext) => {
    const context = requireContext2(rawContext);
    const signalPeer = (peerTopic, signal) => void publishTopic(relay, peerTopic, signal, publishContext2(context, "signal", rootTopic, selfTopic));
    let selfCleanup = null;
    let selfCleanupDone = false;
    let selfSubscriptionP = null;
    let didCleanup = false;
    ...
    const ensureSelfSubscription = () => {
      if (!selfSubscriptionP) selfSubscriptionP = Promise.resolve(subscribeTopic(relay, selfTopic, (topic, msg) => {
        if (!didCleanup) onMessage(topic, msg, signalPeer);
      }, subscriptionContext2(context, "self", rootTopic, selfTopic))).then((cleanup) => {
        selfCleanup = cleanup;
        if (didCleanup) cleanupSelf(cleanup);
      });
      return selfSubscriptionP;
    ...
    const rootCleanup = await subscribeTopic(relay, rootTopic, async (topic, msg) => {
      if (didCleanup) return;
      if (context.isPassive && shouldActivatePassiveRoom2(msg)) await ensureSelfSubscription();
      if (!didCleanup) await onMessage(topic, msg, signalPeer);
    }, subscriptionContext2(context, "root", rootTopic, selfTopic));
    return () => {
      didCleanup = true;
      if (selfCleanup) cleanupSelf(selfCleanup);
      rootCleanup();
  ...
  announce: async (relay, rootTopic, selfTopic, extraPayload, rawContext) => {
    const context = requireContext2(rawContext);
    const result = await publishTopic(relay, rootTopic, toJson3({
      peerId: selfId3,
      ...extraPayload
    }), publishContext2(context, "announce", rootTopic, selfTopic));
    return typeof result === "number" || result !== void 0 && "stopAnnouncing" in result ? result : {
      nextAnnounceMs: (result == null ? void 0 : result.nextAnnounceMs) ?? steadyAnnounceIntervalMs2,
      reannounceOnDisconnect: (result == null ? void 0 : result.reannounceOnDisconnect) ?? reannounceOnDisconnect
    };
  ...
  ...unpublishTopic ? { deactivate: (relay, rootTopic, selfTopic, rawContext) => {
    const context = requireContext2(rawContext);
    return unpublishTopic(relay, rootTopic, publishContext2(context, "announce", rootTopic, selfTopic));
  } } : {}
});

// ../../../../node_modules/@trystero-p2p/mqtt/dist/index.mjs
var defaultRedundancy3 = 4;
var relayManager3 = createRelayManager3((client) => {
  var _a;
  return (_a = client.stream) == null ? void 0 : _a.socket;
});
var msgHandlers3 = relayManager3.scoped();
var subscriptionTokens2 = relayManager3.scoped();
var subscriptionRefs = relayManager3.scoped();
var announcementMessages = relayManager3.scoped();
var joinRoom3 = topic_strategy_default3({
  init: (config) => getRelays3(config, defaultRelayUrls3, defaultRedundancy3).map((url) => {
    const client = relayManager3.register(url, () => mqtt_esm_default.connect(url, {
      queueQoSZero: false,
      resubscribe: false
    }));
    const handlers = msgHandlers3.forRelay(client);
    if (client.listenerCount("message") === 0) client.on("message", (topic, buffer) => {
      var _a;
      return (_a = handlers[topic]) == null ? void 0 : _a.call(handlers, topic, buffer.toString());
    }).on("connect", () => {
      const topics = Object.keys(subscriptionRefs.forRelay(client));
      if (topics.length === 0) return;
      client.subscribeAsync(topics).then(() => {
        Object.entries(announcementMessages.forRelay(client)).forEach(([topic, msg]) => client.publish(topic, msg));
      }).catch(console.error);
  ...
  subscribeTopic: async (client, topic, onMessage) => {
    ...
    if (refs[topic] === 1) await client.subscribeAsync(topic);
    return () => {
...
var ORPNostrSession = class _ORPNostrSession {
  ...
  static async create(roomCode, iceServers) {
    ...
    try {
      ...
      const mqttRoom = joinRoom3({ ...config, brokerUrls: [
  ...
  attachRoom(room, name2) {
    this.rooms.push(room);
    const action = room.makeAction("orp-signal");
    this.sendActions.push((msg, peerId) => {
      try {
        console.log(`[ORP] \u{1F680} Sending msg via ${name2} to ${peerId || "broadcast"}`);
        if (peerId) action.send(msg, { target: peerId });
        else action.send(msg);
      } catch (e) {
        console.warn(`[ORP] Failed to send via ${name2}:`, e);
      }
    });
    room.onPeerJoin = (peerId) => {
      console.log(`[ORP] \u{1F7E2} Peer ${peerId} discovered via ${name2}!`);
    ...
    action.onMessage = (data, meta) => {
      ...
      console.log(`[ORP] \u{1F4E9} Received msg from ${meta.peerId} via ${name2}`);
...
var ORPMqttSession = class _ORPMqttSession {
  ...
  encodeString(str) {
    const encoder4 = new TextEncoder();
    const bytes = encoder4.encode(str);
  ...
  createPublish(message) {
    const encoder4 = new TextEncoder();
    const msgBytes = Array.from(encoder4.encode(message));
    const payload = [
      ...this.encodeString(this.topic),
      ...msgBytes
    ];
